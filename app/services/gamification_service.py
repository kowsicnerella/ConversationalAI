...
class GamificationService:
    ...
    def update_challenge_progress(self, user_id: int, metric: str, value: int = 1) -> List[Dict]:
        ...
        try:
            ...
            challenges = DailyChallenge.query.filter(
                ...
                DailyChallenge.target_metric == metric
            ).all()
    ...
    def _check_achievement_criteria(
        ...
        if achievement.prerequisite_achievement:
            prereq = UserAchievement.query.join(Achievement).filter(
                UserAchievement.user_id == user_id,
                Achievement.achievement_key == achievement.prerequisite_achievement
            ).first()
    ...
    def _unlock_achievement(self, user_id: int, achievement: Achievement) -> Optional[UserAchievement]:
    ...
    def _check_streak_milestones(self, user_id: int, streak: LearningStreak):
