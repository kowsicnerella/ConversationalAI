...
class EnhancedChatTester:
    ...
    def login(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log("User Login", True, f"Logged in as chattest (ID: {self.user_id})")
                return True
        except:
            pass
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                if response.status_code == 200:
                    ...
                    self.auth_token = data.get('access_token')
                    self.user_id = data.get('user', {}).get('id')
                    self.log("User Registration & Login", True, f"Created and logged in as chattest")
                    return True
    ...
    def test_mem0_integration(self):
        ...
        try:
            response = requests.get(f"{BASE_URL}/enhanced-chat/test-mem0", headers=headers)
            ...
            if response.status_code == 200:
                ...
                if mem0_available:
                    ...
                    self.log("Mem0 Integration", True, 
    ...
    def test_create_conversation(self):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                self.log("Create Conversation", True, 
                       f"Created conversation ID: {self.conversation_id}")
                return True
            else:
                self.log("Create Conversation", False, 
        ...
        for i, test_msg in enumerate(test_messages, 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"  - Has Examples: {'✓' if has_examples else '✗'} ({len(ai_response.get('examples', []))} examples)")
                    ...
                    print(f"  - Proficiency: {personalization.get('proficiency_level', 'N/A')}")
                    print(f"  - Memories Used: {personalization.get('memories_used', 0)}")
                    print(f"  - Mem0 Enabled: {context_info.get('mem0_enabled', False)}")
                    
                    self.log(f"Message {i}", True, "Response received with context")
                else:
                    self.log(f"Message {i}", False, 
                           f"Status: {response.status_code}", response.text)
    ...
    def test_conversation_summary(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log("Conversation Summary", True, 
                       f"Analyzed {message_count} messages")
                return True
            else:
                self.log("Conversation Summary", False, 
                       f"Status: {response.status_code}", response.text)
                return False
        except Exception as e:
...
if __name__ == '__main__':
    ...
    print(f"{Fore.YELLOW}Make sure Flask server is running on {BASE_URL}{Style.RESET_ALL}\n")
