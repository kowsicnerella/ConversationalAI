...
class PersonalizationTester:
    ...
    def authenticate(self):
        ...
        try:
            ...
            print(
                "❌ Cannot connect to server. Make sure the Flask app is running on port 5000"
            )
            return False

        # Login
        login_data = {"email": "learner@test.com", "password": "securepass123"}
    ...
    def test_goal_setting(self):
        ...
        goal_data = {"daily_time_goal": 15, "learning_focus": "conversation"}

        try:
            response = self.session.post(
                f"{BASE_URL}/api/personalization/goals", json=goal_data
            )
            ...
            if response.status_code == 201:
                ...
                print(
                    f"   📋 Goal: {data['goal']['daily_time_goal_minutes']} minutes daily"
                )
    ...
    def test_proficiency_assessment(self):
        ...
        try:
            response = self.session.post(
                f"{BASE_URL}/api/personalization/assessment/start"
            )
            ...
            if response.status_code == 201:
                ...
                for i, question in enumerate(questions):
                    ...
                    resp = self.session.post(
                        f"{BASE_URL}/api/personalization/assessment/{assessment_id}/respond",
                        json=response_data,
                    )
                    ...
                    if resp.status_code == 200:
                        ...
                        print(
                            f"   📊 Proficiency: {evaluation.get('proficiency_level', 'N/A')}"
                        )
                        print(
                            f"   🎯 Confidence: {evaluation.get('confidence_score', 0):.2f}"
                        )
                        print(
                            f"   📝 Grammar: {evaluation.get('grammar_score', 0):.2f}"
                        )
                        print(
                            f"   📚 Vocabulary: {evaluation.get('vocabulary_score', 0):.2f}"
                        )
                    else:
                ...
                complete_resp = self.session.post(
                    f"{BASE_URL}/api/personalization/assessment/{assessment_id}/complete"
                )
                ...
                if complete_resp.status_code == 200:
                    ...
                    print(
                        f"   📝 Areas to improve: {', '.join(results['weaknesses'][:3])}"
                    )
                    return True
                else:
    ...
    def test_personalized_dashboard(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   ⏰ Today's Progress: {dashboard['today_time_spent']} minutes"
                )
                print(
                    f"   📈 Goal Progress: {dashboard['goal_progress_percentage']:.1f}%"
                )
    ...
    def test_learning_session(self):
        ...
        try:
            response = self.session.post(
                f"{BASE_URL}/api/personalization/session/start", json=session_data
            )
            ...
            if response.status_code == 201:
                ...
                vocab_words = [
                    {
                        ...
                        "context_sentence": "The sunset is beautiful today.",
                    },
                    {
                        "english_word": "excited",
                        "context_sentence": "I am excited to learn English.",
                    },
                    {
                        "english_word": "wonderful",
                        "context_sentence": "This is a wonderful learning experience.",
                    },
                ]

                for vocab in vocab_words:
                    vocab["session_id"] = session_id
                    vocab_resp = self.session.post(
                        f"{BASE_URL}/api/personalization/vocabulary/track", json=vocab
                    )

                    if vocab_resp.status_code == 201:
                        vocab_data = vocab_resp.json()["vocabulary"]
                        print(
                            f"   ✅ Tracked: {vocab_data['english_word']} = {vocab_data['telugu_translation']}"
                        )
                    else:
                ...
                end_resp = self.session.post(
                    f"{BASE_URL}/api/personalization/session/{session_id}/end",
                    json=end_data,
                )
                ...
                if end_resp.status_code == 200:
                    ...
                    if "session_summary" in summary:
                        ...
                        print(
                            f"   🎉 Achievement: {session_summary.get('achievement', 'N/A')}"
                        )
                        print(
                            f"   📈 Progress: {session_summary.get('progress_note', 'N/A')}"
                        )
                        print(
                            f"   🗨️  Telugu Message: {session_summary.get('telugu_message', 'N/A')}"
                        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if vocabulary:
                    ...
                    print(
                        f"   📝 Practicing word: {first_word['english_word']} = {first_word['telugu_translation']}"
                    )

                    practice_data = {"correct": True, "practice_type": "flashcard"}

                    practice_resp = self.session.post(
                        f"{BASE_URL}/api/personalization/vocabulary/{vocab_id}/practice",
                        json=practice_data,
                    )
                    ...
                    if practice_resp.status_code == 200:
                        ...
                        print(
                            f"   🏆 Mastery Level: {practice_result['mastery_level']}"
                        )
                        print(
                            f"   🔢 Times Practiced: {practice_result['times_practiced']}"
                        )
                    else:
