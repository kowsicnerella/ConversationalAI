...
class LearningPathService:
    ...
    def get_all_paths(difficulty_level=None, category=None, is_active=True):
        ...
        for path in paths:
            ...
            chapters = (
                Chapter.query.filter_by(learning_path_id=path.id, is_active=True)
                .order_by(Chapter.chapter_number)
                .all()
            )
            total_activities = sum(
                len(chapter.activities or []) for chapter in chapters
            )

            # Get enrollment count
            enrollment_count = UserEnrollment.query.filter_by(
    ...
    def get_path_detail(path_id, user_id=None):
        ...
        chapters = (
            Chapter.query.filter_by(learning_path_id=path_id, is_active=True)
            .order_by(Chapter.chapter_number)
            .all()
        )
        ...
        for chapter in chapters:
            ...
            activities = (
                LearningSession.query.filter_by(chapter_id=chapter.id)
            ...
            if enrollment:
                chapter_progress = ChapterProgress.query.filter_by(
                    enrollment_id=enrollment.id, chapter_id=chapter.id
                ).first()
    ...
    def enroll_user(user_id, path_id):
        ...
        existing = UserEnrollment.query.filter_by(
            user_id=user_id, learning_path_id=path_id
        ).first()
        if existing:
            return {
        ...
        total_activities = 0
        for chapter in chapters:
            activity_count = LearningSession.query.filter_by(
    ...
    def get_path_progress(user_id, path_id):
        ...
        chapter_progresses = ChapterProgress.query.filter_by(
            enrollment_id=enrollment.id
        ).all()
        ...
        for cp in chapter_progresses:
            ...
            activity_progresses = (
                ActivityProgress.query.filter_by(chapter_progress_id=cp.id)
                .order_by(ActivityProgress.activity_index)
                .all()
            )
    ...
    def complete_activity(
        user_id, path_id, chapter_id, session_id, score, points_earned, time_spent
    ):
        """
        ...
        chapter_progress = ChapterProgress.query.filter_by(
            enrollment_id=enrollment.id, chapter_id=chapter_id
        ).first()
        if not chapter_progress:
            return {"success": False, "message": "Chapter progress not found"}

        # Get or create activity progress
        activity_progress = ActivityProgress.query.filter_by(
            chapter_progress_id=chapter_progress.id, learning_session_id=session_id
        ).first()
        ...
        scores = [
            ap.score
            for ap in chapter_progress.activity_progress.filter(
        ...
        if chapter_progress.completed_activities >= chapter_progress.total_activities:
            ...
            if next_chapter:
                next_chapter_progress = ChapterProgress.query.filter_by(
                    enrollment_id=enrollment.id, chapter_id=next_chapter.id
                ).first()
        ...
        enrollment.completion_percentage = (
            (enrollment.completed_activities / enrollment.total_activities * 100)
            if enrollment.total_activities > 0
            else 0
        )
        ...
        for cp in enrollment.chapter_progress.all():
            for ap in cp.activity_progress.filter(
                ActivityProgress.is_completed == True
            ).all():
