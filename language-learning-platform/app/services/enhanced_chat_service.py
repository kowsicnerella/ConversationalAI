...
from app.models import Profile, UserActivityLog, VocabularyWord, MistakePattern, ProficiencyAssessment
...
Remember: Your goal is effective learning through clear communication and personalization."""
    ...
    def create_conversation(self, user_id: int, title: Optional[str] = None, 
        ...
        try:
            ...
            if self.mem0_service.is_available():
                context_message = f"Started new {topic or 'general'} learning conversation"
            ...
            db.session.rollback()
            return {'error': f'Failed to create conversation: {str(e)}', 'success': False}
    
    def send_message_with_context(self, conversation_id: int, user_message: str, 
        ...
        try:
            ...
            previous_messages = ChatMessage.query.filter_by(conversation_id=conversation_id)\
            ...
            if not result.get('success'):
                db.session.rollback()
                return {'error': f"AI response failed: {result.get('error')}", 'success': False}
    ...
    def _build_user_context(self, user_id: int, profile: Optional[Profile]) -> Dict[str, Any]:
        ...
        latest_assessment = ProficiencyAssessment.query.filter_by(user_id=user_id)\
            .order_by(ProficiencyAssessment.assessment_date.desc()).first()
        ...
        # Get common mistakes
        recent_mistakes = MistakePattern.query.filter_by(user_id=user_id)\
            .order_by(MistakePattern.last_occurrence.desc()).limit(5).all()
    ...
    def _build_enhanced_prompt(self, context: Dict[str, Any], 
        ...
        # Add user context
        prompt += f"\n\n=== LEARNER PROFILE ===\n"
        prompt += f"Current Level: {context.get('proficiency_level', 'beginner').upper()}\n"
        prompt += f"Learning Style: {context.get('learning_style', 'visual').title()}\n"
        prompt += f"Vocabulary Level: {context.get('vocabulary_level', 'basic').title()}\n"
    ...
    def _parse_enhanced_response(self, response_text: str) -> Dict[str, Any]:
        ...
        for i, line in enumerate(lines):
            ...
            # Examples
            if any(marker in line.lower() for marker in ['example:', 'e.g.', 'for instance']):
                example = re.sub(r'^(example|e\.g\.|for instance)[:\s]*', '', line, flags=re.I).strip()
                if example and len(example) > 10:
                    examples.append(example)
                # Check next line if current is just label
                if len(example) < 10 and i + 1 < len(lines):
                    next_line = lines[i + 1].strip()
                    if next_line and not any(kw in next_line.lower() for kw in ['example', 'tip', 'note', 'grammar']):
            ...
            # Numbered or bulleted examples
                ...
                if not any(kw in example.lower() for kw in ['tip:', 'note:', 'grammar:']):
                    examples.append(example)
            
            # Grammar explanations
            if any(kw in line.lower() for kw in ['grammar:', 'rule:', 'structure:', 'form:']):
                grammar_lines.append(line)
            
            # Tips
            if any(kw in line.lower() for kw in ['tip:', 'remember:', 'note:', 'important:']):
                tip = re.sub(r'^(tip|remember|note|important)[:\s]*', '', line, flags=re.I).strip()
                if tip:
                    tips.append(tip)
    ...
    def _store_interaction_in_memory(self, user_id: int, user_message: str,
        ...
        try:
            ...
            if parsed_data.get('grammar_explanation'):
                memory_content += f"Grammar topic: {parsed_data['grammar_explanation'][:100]}\n"
    ...
    def _extract_and_save_vocabulary(self, user_id: int, user_message: str,
                                    ai_response: str, parsed_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            for word in vocabulary_words[:5]:  # Limit to 5 per interaction
                ...
                if not existing:
                    ...
                    if match:
                        telugu_match = re.search(r'\([^)]*[\u0C00-\u0C7F]+[^)]*\)', match.group())
                        if telugu_match:
