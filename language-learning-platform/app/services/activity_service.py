...
class ActivityService:
    ...
    def _generate_ai_content_with_retry(self, prompt, content_type="content"):
        ...
        for attempt in range(self.max_retries):
            try:
                print(f"Generating {content_type} with AI (attempt {attempt + 1}/{self.max_retries})...")
    ...
    def generate_quiz(self, user_id, topic="daily routine", level="beginner", num_questions=5):
        """
...
"""
    ...
    def generate_flashcards(self, user_id, topic="food", level="beginner", num_cards=10):
        """
...
"""
    ...
    def evaluate_activity_submission(self, user_id, activity_type, activity_data, user_answers):
        """
        ...
        try:
            if activity_type == "quiz":
                ...
                return self.evaluate_writing(user_id, activity_data, user_answers.get('user_text', ''))
            else:
    ...
    def _evaluate_flashcards(self, user_id, flashcard_data, user_responses):
        ...
        known_count = len([r for r in user_responses.get('responses', []) if r.get('marked_as_known')])
    ...
    def _save_vocabulary_from_flashcards(self, user_id, flashcards, user_responses):
        ...
        try:
            responses_dict = {r.get('card_id'): r for r in user_responses.get('responses', [])}
    ...
    def _extract_key_words(self, text):
        ...
        common_words = {'the', 'a', 'an', 'is', 'are', 'was', 'were', 'do', 'does', 'did', 'you', 'i', 'me', 'my'}
...
"""
            ...
            quality_bonus = int((feedback_data.get('overall_score', 0) / 100) * 30)  # Up to 30 bonus
    ...
    def generate_role_playing_scenario(self, user_id, topic="restaurant", level="beginner"):
        """
...
"""
            ...
            scenario_data = self._generate_ai_content_with_retry(prompt, "role-play scenario")
            
            scenario_data['generated_at'] = datetime.utcnow().isoformat()
            scenario_data['user_id'] = user_id
            scenario_data['conversation_history'] = []  # Initialize empty conversation history
    ...
    def generate_conversation_response(self, user_id, scenario_data, conversation_history, user_message):
        """
...
"""
            ...
            response_data = self._generate_ai_content_with_retry(prompt, "conversation response")
            
            response_data['timestamp'] = datetime.utcnow().isoformat()
            
            # Extract vocabulary from user's message
            self._save_vocabulary_from_conversation(user_id, user_message, scenario_data.get('topic', ''))
...
"""
            ...
            evaluation = self._generate_ai_content_with_retry(prompt, "role-play evaluation")
            
            # Calculate points: base 30 + quality bonus
            base_points = 30
            quality_bonus = int((evaluation.get('overall_score', 70) / 100) * 20)  # 0-20 bonus
