...
class AdaptiveLearningAlgorithm:
    ...
    def analyze_user_performance(self, user_id: int, days: int = 7) -> Dict:
        ...
        avg_efficiency = (
            sum(time_efficiency) / len(time_efficiency) if time_efficiency else 0
        )

        # Calculate consistency (lower standard deviation = more consistent)
        if len(consistency_scores) > 1:
            mean_score = sum(consistency_scores) / len(consistency_scores)
            variance = sum((x - mean_score) ** 2 for x in consistency_scores) / len(
                consistency_scores
            )
        ...
        return {
            ...
            "performance_trend": self._calculate_performance_trend(
                recent_logs, activity_dict
            ),
        }

    def recommend_next_activities(
        self, user_id: int, learning_path_id: Optional[int] = None, count: int = 5
    ) -> List[Dict]:
        """
        ...
        if learning_path_id:
            available_activities = self._get_learning_path_activities(
    ...
    def assess_concept_mastery(
        self, user_id: int, skill_area: str, concept: str
    ) -> Dict:
        """
        ...
        try:
            ...
            if len(recent_logs) < self.MIN_ACTIVITIES_FOR_ASSESSMENT:
                return {
                    ...
                    "activities_needed": self.MIN_ACTIVITIES_FOR_ASSESSMENT
                    - len(recent_logs),
                }

            # Calculate mastery metrics
            scores = [
                log.score / log.max_score for log in recent_logs if log.max_score > 0
            ]
            if not scores:
            ...
            if (
                avg_score >= self.MASTERY_THRESHOLD
                and consistency >= 0.8
                and recent_trend >= 0
            ):
                ...
                avg_score >= self.DIFFICULTY_ADJUSTMENT_THRESHOLD and consistency >= 0.6
            ):
            ...
            return {
                ...
                "recommendation": self._get_mastery_recommendation(
                    mastery_status, avg_score
                ),
            }
    ...
    def implement_spaced_repetition(self, user_id: int, concept: str) -> Dict:
        ...
        try:
            ...
            performance_level = (
                last_activity.score / last_activity.max_score
                if last_activity.max_score > 0
                else 0.5
            )
            ...
            if hours_since >= adjusted_interval:
                ...
                next_review_time = last_activity.completed_at + timedelta(
                    hours=adjusted_interval
                )
                return {
    ...
    def detect_learning_difficulties(
        self, user_id: int, skill_area: str = None
    ) -> Dict:
        """
        ...
        try:
            ...
            recent_logs = (
                query.order_by(UserActivityLog.completed_at.desc()).limit(20).all()
            )
            ...
            if len(scores) >= 5:
                ...
                earlier_avg = (
                    sum(scores[5:10]) / min(5, len(scores) - 5)
                    if len(scores) > 5
                    else recent_avg
                )
            ...
            time_spent = [
                log.time_spent_minutes for log in recent_logs if log.time_spent_minutes
            ]
            if time_spent:
    ...
    def _generate_support_message(self, difficulties: List[str]) -> str:
        ...
        return messages.get(
            primary_difficulty,
            "We're here to help you succeed! Let's adjust your learning plan.",
        )

    def adjust_difficulty_dynamically(
        self, user_id: int, activity_id: int, user_performance: Dict
    ) -> Dict:
        """
        ...
        activity_type_performance = performance_analysis[
            "activity_type_performance"
        ].get(activity.activity_type, 0.5)
        ...
        if (
            user_accuracy >= self.MASTERY_THRESHOLD
            and activity_type_performance >= self.MASTERY_THRESHOLD
        ):
            ...
            if current_difficulty == "beginner":
                ...
                adjustment_reason = (
                    "Mastery demonstrated - ready for intermediate level"
                )
            elif current_difficulty == "intermediate":
                new_difficulty = "advanced"
                adjustment_needed = True
                adjustment_reason = (
                    "Excellent performance - advancing to advanced level"
                )
            ...
            if current_difficulty == "expert":
                ...
                adjustment_needed = True
                adjustment_reason = (
                    "Providing more foundation - moving to advanced level"
                )
            elif current_difficulty == "advanced":
                new_difficulty = "intermediate"
                adjustment_needed = True
                adjustment_reason = (
                    "Building confidence - returning to intermediate level"
                )
            elif current_difficulty == "intermediate":
    ...
    def identify_learning_gaps(self, user_id: int) -> Dict:
        ...
        return {
            ...
            "recommended_interventions": interventions,
            "overall_assessment": self._generate_overall_gap_assessment(
    ...
    def personalize_learning_pace(self, user_id: int) -> Dict:
        ...
        recent_logs = (
            UserActivityLog.query.filter_by(user_id=user_id)
            .filter(
                UserActivityLog.completed_at >= datetime.utcnow() - timedelta(days=21)
            )
            .order_by(UserActivityLog.completed_at.desc())
            .all()
        )
        ...
        return {
            ...
            "pace_adjustments": self._generate_pace_adjustments(
                optimal_pace, performance
            ),
            "motivation_strategies": self._suggest_motivation_strategies(
                user_id, optimal_pace
            ),
        }

    def generate_adaptive_exercise(
        self, user_id: int, topic: str, skill_area: str
    ) -> Dict:
        """
        ...
        skill_performance = performance["activity_type_performance"].get(
            skill_area, 0.5
        )
        ...
        return {
            ...
            "adaptation_notes": {
                ...
                "support_level": (
                    "high"
                    if skill_performance < 0.6
                    else "medium" if skill_performance < 0.8 else "low"
                ),
            },
        }
    ...
    def _extract_user_preferences(self, user: User) -> Dict:
        ...
        if hasattr(user, "enrollment_data") and user.enrollment_data:
            ...
            if isinstance(enrollment_data, dict):
                preferences.update(
                    {
                        "preferred_difficulty": enrollment_data.get(
                            "difficulty_preference", "beginner"
                        ),
                        "learning_goals": enrollment_data.get(
                            "learning_goals", ["conversation"]
                        ),
                        "time_availability": enrollment_data.get(
    ...
    def _identify_knowledge_gaps(self, user_id: int, performance: Dict) -> List[Dict]:
        ...
        for activity_type, avg_score in performance.get(
            "activity_type_performance", {}
        ).items():
        ...
        for difficulty, avg_score in performance.get(
            "difficulty_performance", {}
        ).items():
            if avg_score < self.STRUGGLE_THRESHOLD:
                gaps.append(
    ...
    def _get_learning_path_activities(
        self, learning_path_id: int, user_id: int
    ) -> List:
        """Get available activities from a specific learning path."""
        # Get activities user hasn't completed yet
        completed_activity_ids = [
            log.activity_id
            for log in UserActivityLog.query.filter_by(
                user_id=user_id, learning_path_id=learning_path_id
            ).all()
        ]

        activities = (
            Activity.query.filter_by(learning_path_id=learning_path_id)
            .filter(~Activity.id.in_(completed_activity_ids))
            .order_by(Activity.order_in_path)
            .limit(10)
            .all()
        )
    ...
    def _get_general_activities(self, user_id: int) -> List:
        ...
        completed_activity_ids = [
            log.activity_id
            for log in UserActivityLog.query.filter_by(user_id=user_id).all()
        ]

        activities = (
            Activity.query.filter(~Activity.id.in_(completed_activity_ids))
            .limit(20)
            .all()
        )
    ...
    def _score_activity_recommendation(
        self,
        activity: Activity,
        performance: Dict,
        knowledge_gaps: List,
        ...
        activity_type_performance = performance["activity_type_performance"].get(
        ...
        for gap in knowledge_gaps:
            if (
                gap["type"] == "activity_type" and gap["area"] == activity.activity_type
            ) or (
                gap["type"] == "difficulty_level"
                and gap["area"] == activity.difficulty_level
            ):
                score += 25
                break

        # Time availability match (10% weight)
        if activity.estimated_duration_minutes <= preferences.get(
            "time_availability", 30
        ):
            score += 10

        # Variety bonus (10% weight) - encourage trying different types
        if activity.activity_type not in performance.get(
            "activity_type_performance", {}
        ):
            score += 10
    ...
    def _get_recommendation_reasons(
        ...
        if activity.difficulty_level == optimal_difficulty:
            reasons.append(
                f"Matches your optimal difficulty level ({optimal_difficulty})"
            )
        ...
        if activity_type_performance < 0.6:
            reasons.append(
                f"Helps improve {activity.activity_type} skills (current: {activity_type_performance:.1%})"
            )

        for gap in knowledge_gaps:
            if gap["area"] == activity.activity_type:
                reasons.append(
                    f"Addresses identified weakness in {activity.activity_type}"
                )
                break
    ...
    def _generate_adaptive_content_suggestions(
        self,
        user_id: int,
        activity: Activity,
        new_difficulty: str,
    ...
    def _get_telugu_difficulty_explanation(
        self, adjustment_needed: bool, current_difficulty: str, new_difficulty: str
    ) -> str:
    ...
    def _analyze_vocabulary_gaps(self, user_id: int) -> Dict:
        ...
        return {
            "gap_level": "medium",
            "areas": ["business_terms", "daily_conversation"],
        }
    ...
    def _prioritize_learning_gaps(
        self, skill_gaps: Dict, performance: Dict
    ) -> List[Dict]:
        ...
        for skill, gap_data in skill_gaps.items():
            ...
            prioritized.append(
                {"skill": skill, "gap_data": gap_data, "priority_score": priority_score}
            )
    ...
    def _generate_overall_gap_assessment(
        self, skill_gaps: Dict, performance: Dict
    ) -> Dict:
        """Generate overall assessment of learning gaps."""
        high_gaps = [
            skill for skill, data in skill_gaps.items() if data["gap_level"] == "high"
        ]
        medium_gaps = [
            skill for skill, data in skill_gaps.items() if data["gap_level"] == "medium"
        ]
        ...
        assessment = {
            ...
            "confidence_level": (
                "high" if performance.get("total_activities", 0) > 20 else "medium"
            ),
        }

        return assessment

    def _calculate_optimal_learning_pace(
        self,
        avg_sessions: float,
        avg_length: float,
        consistency: float,
    ...
    def _generate_pace_adjustments(
        self, optimal_pace: Dict, performance: Dict
    ) -> List[Dict]:
        """Generate specific pace adjustment recommendations."""
        adjustments = []

        current_intensity = (
            performance.get("total_activities", 0) / 7
        )  # Activities per day
    ...
    def _suggest_motivation_strategies(
        self, user_id: int, optimal_pace: Dict
    ) -> List[str]:
