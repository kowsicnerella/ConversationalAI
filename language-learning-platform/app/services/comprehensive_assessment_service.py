...
class ComprehensiveAssessmentService:
    ...
    def conduct_comprehensive_assessment(self, user_id: int) -> Dict:
        ...
        try:
            ...
            assessment.assessment_data = {
                ...
                "questions_by_skill": self._categorize_questions_by_skill(
    ...
    def evaluate_assessment_response(
        ...
        try:
            ...
            # Find the question
            question = next(
                (q for q in assessment.questions_asked if q["id"] == question_id), None
            )
            if not question:
            ...
            if question["type"] == "multiple_choice":
                score, feedback = self._evaluate_multiple_choice(
                ...
                score, feedback = self._evaluate_writing_response(
                    question, user_response
                )
            elif question["type"] == "reading_comprehension":
                score, feedback = self._evaluate_reading_response(
                    question, user_response
                )
            elif question["type"] == "sentence_correction":
                score, feedback = self._evaluate_grammar_correction(
                    question, user_response
                )
            elif question["type"] in ["word_meaning", "synonym"]:
                score, feedback = self._evaluate_vocabulary_response(
                    question, user_response
                )
    ...
    def _evaluate_multiple_choice(
        self, question: Dict, user_response: str
    ) -> Tuple[float, str]:
    ...
    def _evaluate_writing_response(
        self, question: Dict, user_response: str
    ) -> Tuple[float, str]:
        ...
        try:
            ...
            if word_count < min_words:
                ...
                feedback_parts.append(
                    f"Response is too short ({word_count} words). Try to write at least {min_words} words."
                )
            elif word_count > max_words:
                score -= 0.1
                feedback_parts.append(
                    f"Response is too long ({word_count} words). Try to keep it under {max_words} words."
                )
            else:
            ...
            if user_response.count(".") == 0:
                score -= 0.1
                feedback_parts.append(
                    "Remember to end sentences with proper punctuation."
                )
    ...
    def _evaluate_grammar_correction(
        ...
        if correct_answer in user_answer or user_answer in correct_answer:
            ...
            return (
                0.3,
                f"The correction needs work. Correct answer: {question.get('correct_answer')}",
            )

    def _evaluate_vocabulary_response(
        self, question: Dict, user_response: str
    ) -> Tuple[float, str]:
    ...
    def _evaluate_context_usage(
        ...
        if target_word.lower() in user_response.lower():
            return (
                0.8,
                f"Good use of '{target_word}' in context. The sentence shows understanding of the word's meaning.",
            )
        else:
    ...
    def finalize_assessment(self, assessment_id: int) -> Dict:
        ...
        try:
            ...
            overall_score = sum(
                skill_scores[skill] * self.SKILL_WEIGHTS[skill]
                for skill in skill_scores
                if skill_scores[skill] is not None
            )
            ...
            return {
                ...
                "recommendations": self._generate_learning_recommendations(
                    strengths, weaknesses, proficiency_level
                ),
    ...
    def _calculate_skill_score(
        self, assessment: ProficiencyAssessment, skill_area: str
    ) -> Optional[float]:
    ...
    def _generate_learning_recommendations(
        self, strengths: List[str], weaknesses: List[str], proficiency_level: str
    ) -> List[str]:
        ...
        if "reading" in weaknesses:
            recommendations.append(
                "Focus on daily reading practice with graded materials"
            )
        ...
        if "grammar" in weaknesses:
            recommendations.append(
                "Study fundamental grammar rules with interactive exercises"
            )
        if "vocabulary" in weaknesses:
            recommendations.append(
                "Build vocabulary through contextual learning and flashcards"
            )
        if "listening" in weaknesses:
            recommendations.append(
                "Improve listening skills with audio content and conversation practice"
            )
        if "speaking" in weaknesses:
            recommendations.append(
                "Practice speaking through role-play scenarios and pronunciation exercises"
            )

        # Add level-specific recommendations
        if proficiency_level == "beginner":
            recommendations.append(
                "Start with basic conversation patterns and everyday vocabulary"
            )
            ...
            recommendations.append(
                "Practice advanced writing and professional communication"
            )
