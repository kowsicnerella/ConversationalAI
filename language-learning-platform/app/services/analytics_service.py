...
class AnalyticsService:
    ...
    @staticmethod
    def record_assessment_question_response(
        assessment_id,
        user_id,
        question_data,
        user_answer,
        ...
        try:
            ...
            # Update analytics
            AnalyticsService.update_skill_analytics(
                user_id, question_data.get("skill_area"), is_correct, time_spent
            )
    ...
    @staticmethod
    def record_activity_question_response(
        activity_log_id,
        user_id,
        activity_id,
        question_index,
        ...
        try:
            ...
            if existing:
                ...
                existing.metric_value = (
                    (existing.metric_value * existing.session_count) + metric_value
                ) / new_count
    ...
    def update_learning_streaks(user_id, activity_type=None, skill_area=None):
        ...
        try:
            ...
            # Update daily streak
            AnalyticsService._update_streak(
                user_id, "daily", today, activity_type, skill_area
            )

            # Update activity-specific streak if provided
            if activity_type:
                AnalyticsService._update_streak(
                    user_id, "activity_specific", today, activity_type, skill_area
                )

            # Update skill-specific streak if provided
            if skill_area:
                AnalyticsService._update_streak(
                    user_id, "skill_specific", today, activity_type, skill_area
                )
    ...
    @staticmethod
    def _update_streak(
        user_id, streak_type, activity_date, activity_type=None, skill_area=None
    ):
    ...
    @staticmethod
    def track_ai_generated_content(
        content_type,
        content_data,
    ...
    def calculate_performance_trends(user_id, period="weekly"):
        ...
        try:
            ...
            existing_trend = PerformanceTrend.query.filter_by(
                user_id=user_id, trend_period=period, period_start=period_start
            ).first()
            ...
            if analytics_data:
                ...
                accuracy_values = [
                    ...
                    if a.metric_type == "accuracy"
                ]
                speed_values = [
                    a.metric_value for a in analytics_data if a.metric_type == "speed"
                ]
                ...
                if "vocabulary" in skill_data:
                    trend.vocabulary_progress = sum(skill_data["vocabulary"]) / len(
                        skill_data["vocabulary"]
                    )
                if "grammar" in skill_data:
                    trend.grammar_progress = sum(skill_data["grammar"]) / len(
                        skill_data["grammar"]
                    )
                if "reading" in skill_data:
                    trend.reading_progress = sum(skill_data["reading"]) / len(
                        skill_data["reading"]
                    )
                if "conversation" in skill_data:
                    trend.conversation_progress = sum(skill_data["conversation"]) / len(
                        skill_data["conversation"]
                    )
