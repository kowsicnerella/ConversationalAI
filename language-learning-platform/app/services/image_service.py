...
class ImageService:
    ...
    def validate_image(file):
        ...
        if file_ext not in ImageService.ALLOWED_FORMATS:
            return (
                False,
                f"Invalid file format. Allowed: {', '.join(ImageService.ALLOWED_FORMATS)}",
                None,
            )
    ...
    def analyze_image_for_learning(image_path, user_id):
        ...
        try:
            ...
            return {
                ...
                "raw_response": (
                    result.get("analysis") if "result" in locals() else None
                ),
            }
    ...
    @staticmethod
    def create_image_learning_record(
        user_id,
        file_info,
        image_path,
        filename,
        analysis_result,
        is_camera_capture=False,
        device_type="desktop",
    ):
        """
        ...
        try:
            ...
            for idx in object_indices:
                ...
                existing_word = VocabularyWord.query.filter_by(
                    user_id=user_id, word_english=obj["object_name_english"]
                ).first()
    ...
    def get_user_image_history(user_id, limit=20):
        ...
        try:
            records = (
                ImageLearning.query.filter_by(user_id=user_id)
