...
class GoalService:
    ...
    def get_available_goals(user_id):
        ...
        try:
            ...
            active_goal_ids = [
                g.goal_type_id for g in user_active_goals if g.goal_type_id
            ]
            ...
            for goal_type in all_goal_types:
                ...
                goal_dict["is_recommended"] = GoalService._is_recommended_for_user(
                    user, goal_type
                )
    ...
    def _is_recommended_for_user(user, goal_type):
        ...
        return goal_type.difficulty_level in level_progression.get(
            user_level, ["beginner"]
        )
        ...
        try:
            ...
            for criterion_type, target_value in criteria.items():
                ...
                if criterion_type == "activities_completed":
                    ...
                    criterion_type == "vocabulary_learned"
                    or criterion_type == "vocabulary_mastered"
                ):
                    milestone = Milestone(
                        user_id=user_id,
                        user_goal_id=goal_id,
            ...
            for goal in active_goals:
                ...
                if updated:
                    ...
                    goal.progress_percentage = (
                        GoalService._calculate_progress_percentage(
                            goal.criteria, current_progress
                        )
                    )
    ...
    def _update_milestones(goal, progress_data):
        ...
        try:
            milestones = Milestone.query.filter_by(
                user_goal_id=goal.id, is_completed=False
            ).all()

            for milestone in milestones:
                criterion_key = GoalService._get_criterion_key_for_type(
                    milestone.criteria_type
                )
    ...
    def complete_goal(goal_id):
        ...
        try:
            ...
            goal_template = GoalService.GOAL_TEMPLATES.get(
                goal.goal_type.name if goal.goal_type else None
            )
            if goal_template and "target_level" in goal_template:
                ...
                if target_level != user.current_level:
                    GoalService._level_up(
                        user.id, target_level, "goal_completion", goal.id
                    )
    ...
    def _generate_certificate(user_id, goal_id):
        ...
        try:
            ...
            certificate = Certificate(
                ...
                level_achieved=(
                    goal.goal_type.difficulty_level if goal.goal_type else "beginner"
                ),
            ...
            certificate.thumbnail_url = (
                f"/api/goals/certificates/{certificate.id}/thumbnail"
            )
            db.session.commit()
    ...
    def get_goal_detail(goal_id, include_milestones=True):
        ...
        try:
            ...
            if include_milestones:
                milestones = (
                    Milestone.query.filter_by(user_goal_id=goal_id)
    ...
    def get_level_progression_history(user_id):
        ...
        try:
            progressions = (
                LevelProgression.query.filter_by(user_id=user_id)
