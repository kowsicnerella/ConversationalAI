...
def get_all_chapters():
    ...
    try:
        ...
        user_progress = {
            prog.chapter_id: prog
            for prog in UserChapterProgress.query.filter_by(user_id=user_id)
            .filter(UserChapterProgress.chapter_id.in_(chapter_ids))
            .all()
        }
...
def get_chapter_details(chapter_id):
    ...
    try:
        ...
        recent_sessions = (
            PracticeSession.query.filter_by(user_id=user_id, chapter_id=chapter_id)
...
def start_practice_session(chapter_id):
    ...
    try:
        ...
        if not is_unlocked:
            return (
                jsonify(
                    {"error": "Chapter is locked", "telugu_message": "అధ్యాయం లాక్ చేయబడింది"}
                ),
                403,
            )
                ...
                jsonify(
                    {
                        ...
                        "session": {
                            ...
                            "current_questions": len(
                                active_session.questions_data or []
                            ),
    ...
    try:
        ...
        for dep in dependencies:
            if dep.is_strict:
                ...
                progress = UserChapterProgress.query.filter_by(
                    user_id=user_id, chapter_id=dep.prerequisite_chapter_id
                ).first()
...
def get_progress_graph():
    ...
    try:
        ...
        # Get all chapters
        chapters = (
            Chapter.query.filter_by(is_active=True)
        ...
        user_progress = {
            prog.chapter_id: prog
            for prog in UserChapterProgress.query.filter_by(user_id=user_id).all()
        }
...
def create_custom_chapter():
    ...
    try:
        ...
        new_chapter = Chapter(
            title=title,
            description=chapter_content.get(
                "description", f"Custom chapter on {topic}"
            ),
            chapter_number=9999,  # Custom chapters get high numbers
            difficulty_level=difficulty_level,
            topic=topic,
            subtopics=subtopics,
            estimated_duration_minutes=chapter_content.get(
                "estimated_duration_minutes", 45
            ),
            ...
            jsonify(
                {
                    ...
                    "user_performance": {
                        "best_score": (
                            user_progress.best_score if user_progress else 0.0
                        ),
                        "status": (
                            user_progress.status if user_progress else "not_started"
                        ),
                        "total_attempts": (
                            user_progress.total_attempts if user_progress else 0
                        ),
                    },
                    ...
                    "summary": {
                        ...
                        "completed_chapters": len(
                            [
                                ...
                                if ch["user_progress"]["status"] == "completed"
                            ]
                        ),
                        "unlocked_chapters": len(
                            [ch for ch in chapters_data if ch["is_unlocked"]]
                        ),
                    },
                }
            ),
...
def unlock_next_chapter():
    ...
    try:
        ...
        completed_chapters = [
            p.chapter_id for p in user_progress if p.status in ["completed", "mastered"]
        ]
        ...
        if current_chapter_id:
            ...
            next_chapter = (
                Chapter.query.filter(
                    ~Chapter.id.in_(completed_chapters), Chapter.is_active == True
                )
                .order_by(Chapter.chapter_number)
                .first()
            )
        ...
        unlock_status = {
            ...
            "is_unlocked": is_unlocked,
            "unlock_message": (
                "Chapter unlocked! Ready to start."
                if is_unlocked
                else "Complete prerequisites to unlock."
            ),
            "telugu_unlock_message": (
                "అధ్యాయం అన్‌లాక్ చేయబడింది! ప్రారంభించడానికి సిద్ధం."
                if is_unlocked
                else "అన్‌లాక్ చేయడానికి ముందుగా అవసరమైన అధ్యాయాలు పూర్తి చేయండి."
            ),
        }
