...
def get_available_goals():
    ...
    try:
        ...
        return (
            jsonify({"success": True, "available_goals": goals, "total": len(goals)}),
            200,
        )
    ...
    try:
        ...
        if data.get("is_custom"):
            ...
            if not data.get("goal_type_id"):
                return (
                    jsonify({"error": "goal_type_id is required for template goals"}),
                    400,
                )
...
def update_goal_progress(goal_id):
    ...
    try:
        ...
        return (
            jsonify({"success": True, "message": "Progress updated successfully!"}),
            200,
        )
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def abandon_goal(goal_id):
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Goal abandoned"}), 200
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def get_certificate_detail(certificate_id):
    ...
    try:
        ...
        return jsonify({"success": True, "certificate": certificate.to_dict()}), 200
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def verify_certificate(certificate_number):
    ...
    try:
        ...
        if not certificate.is_public:
            return (
                jsonify({"valid": False, "message": "This certificate is private"}),
                403,
            )
            ...
            jsonify(
                {
                    ...
                    "certificate": {
                        ...
                        "issued_date": (
                            certificate.issued_date.isoformat()
...
def get_goal_statistics():
    ...
    try:
        ...
        active_count = UserGoal.query.filter_by(
            user_id=user_id, status="active"
        ).count()
        completed_count = UserGoal.query.filter_by(
