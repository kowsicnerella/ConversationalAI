...
def complete_lesson():
    ...
    try:
        ...
        if "error" in review_result:
            return jsonify({"success": False, "error": review_result["error"]}), 500
            ...
            jsonify(
                {
                    ...
                    "telugu_motivational_message": review_result.get(
                        "telugu_motivational_message"
                    ),
                    "next_lesson": (
                        next_lesson.get("lesson_plan")
                        if "lesson_plan" in next_lesson
                        else None
                    ),
...
def get_user_reviews():
    ...
    try:
        ...
        return (
            jsonify({"success": True, "reviews": reviews, "count": len(reviews)}),
            200,
        )
    ...
    try:
        ...
        if score >= 100:
            ...
            existing = Milestone.query.filter_by(
                user_id=user_id, milestone_type="perfect_lesson"
            ).first()
        ...
        if profile and profile.current_streak == 7:
            existing = Milestone.query.filter_by(
                user_id=user_id, milestone_type="week_streak"
            ).first()
        ...
        if profile and profile.mastery_metrics:
            ...
            for threshold, m_type, title, telugu_title, points in mastery_thresholds:
                if overall_mastery >= threshold:
                    existing = Milestone.query.filter_by(
                        user_id=user_id, milestone_type=m_type
                    ).first()
