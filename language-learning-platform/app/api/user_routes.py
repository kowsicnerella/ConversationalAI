...
def get_profile():
    ...
    try:
        ...
        if "error" in profile:
            return (
                jsonify(
                    {"error": profile["error"], "telugu_message": "ప్రొఫైల్ డేటా లభించడంలో లోపం"}
                ),
                500,
            )
    ...
    try:
        ...
        settings = {
            "user_info": {
                ...
                "email": user.email,
                "native_language": (
                    user.profile.native_language if user.profile else "Telugu"
                ),
                "target_language": (
                    user.profile.target_language if user.profile else "English"
                ),
                "joined_date": user.created_at.isoformat(),
            },
            "learning_preferences": {
                "daily_time_goal": (
                    current_goal.daily_time_goal_minutes if current_goal else 10
                ),
                "learning_focus": (
                    current_goal.learning_focus if current_goal else "conversation"
                ),
...
def get_user_statistics():
    ...
    try:
        ...
        mastered_words = VocabularyWord.query.filter(
            ...
            >= 0.8,  # Consider words with 80%+ mastery as "mastered"
        ).count()

        # Get weekly stats
        week_ago = datetime.utcnow() - timedelta(days=7)
        weekly_sessions = LearningSession.query.filter(
            LearningSession.user_id == user_id, LearningSession.start_time >= week_ago
        ).count()
        ...
        statistics = {
            ...
            "this_week": {
                ...
                "average_session_length": (
                    round(weekly_time / weekly_sessions, 1)
...
def delete_account():
    ...
    try:
        ...
        if not check_password_hash(user.password_hash, password):
            return (
                jsonify(
                    {"error": "Incorrect password", "telugu_message": "తప్పు పాస్‌వర్డ్"}
                ),
                400,
            )
...
def get_learning_paths(user_id):
    ...
    try:
        ...
        return (
            jsonify({"error": "Failed to fetch learning paths", "details": str(e)}),
            500,
        )
...
def create_learning_path():
    ...
    try:
        ...
        return (
            jsonify({"error": "Failed to create learning path", "details": str(e)}),
            500,
        )
...
def log_activity_completion():
    ...
    try:
        ...
        return (
            jsonify({"error": "Failed to log activity completion", "details": str(e)}),
            500,
        )
