...
def get_assessment_question_analysis(assessment_id):
    ...
    try:
        ...
        question_responses = (
            AssessmentQuestionResponse.query.filter_by(
                assessment_id=assessment_id, user_id=user_id
            )
            .order_by(AssessmentQuestionResponse.question_id)
            .all()
        )
        ...
        stats = {
            ...
            "average_time_per_question": (
                total_time / len(question_responses) if question_responses else 0
            ),
            "total_hints_used": sum(r.hints_used or 0 for r in question_responses),
            "average_confidence": (
                sum(r.confidence_level or 0 for r in question_responses)
        ...
        for skill, data in skill_breakdown.items():
            ...
            data["average_time"] = (
                data["total_time"] / data["total"] if data["total"] > 0 else 0
            )
        ...
        return (
            jsonify({"error": "Failed to get question analysis", "details": str(e)}),
            500,
        )


@enhanced_assessment_bp.route(
    "/<int:assessment_id>/comparative-report", methods=["GET"]
)
@jwt_required()
def get_comparative_assessment_report(assessment_id):
    ...
    try:
                ...
                jsonify(
                    {
                        ...
                        "current_assessment": {
                            ...
                            "completed_at": (
                                current_assessment.completed_at.isoformat()
                                if current_assessment.completed_at
                                else None
                            ),
                        },
                    }
        ...
        current_percentage = (
            (current_assessment.score / current_assessment.max_score * 100)
            if current_assessment.max_score > 0
            else 0
        )
        previous_percentage = (
            (most_recent.score / most_recent.max_score * 100)
            if most_recent.max_score > 0
            else 0
        )
            ...
            jsonify(
                {
                    ...
                    "comparison_with_previous": {
                        ...
                        "improvement_status": (
                            "improved"
                            if improvement > 0
                            else "declined" if improvement < 0 else "same"
                        ),
                    },
        ...
        return (
            jsonify(
                {"error": "Failed to generate comparative report", "details": str(e)}
            ),
            500,
        )
...
def get_skill_progression():
    ...
    try:
        ...
        assessments = (
            ...
            .order_by(ProficiencyAssessment.completed_at)
            .all()
        )
        ...
        return (
            jsonify({"error": "Failed to get skill progression", "details": str(e)}),
            500,
        )
...
def get_activity_question_breakdown(activity_id):
    ...
    try:
        ...
        activity_logs = (
            UserActivityLog.query.filter_by(user_id=user_id, activity_id=activity_id)
            .order_by(desc(UserActivityLog.completed_at))
            .all()
        )
        ...
        for log in activity_logs:
            responses = (
                ActivityQuestionResponse.query.filter_by(
                    activity_log_id=log.id, user_id=user_id, activity_id=activity_id
                )
                .order_by(ActivityQuestionResponse.question_index)
                .all()
            )
...
def get_activity_performance_history():
    ...
    try:
        ...
        # Build query
        query = (
            db.session.query(UserActivityLog, Activity)
            .join(Activity)
            .filter(
        ...
        for log, activity in results:
            score_percentage = (
                (log.score / log.max_score * 100)
                if log.max_score and log.max_score > 0
                else 0
            )
        ...
        return (
            jsonify({"error": "Failed to get performance history", "details": str(e)}),
            500,
        )
