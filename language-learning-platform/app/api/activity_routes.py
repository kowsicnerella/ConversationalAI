...
def generate_quiz():
    ...
    try:
        ...
        if "error" in quiz_content:
            return (
                jsonify({"error": "Failed to generate quiz", "details": quiz_content}),
                500,
            )
...
def generate_reading():
    ...
    try:
        ...
        if "error" in reading_content:
            return (
                jsonify(
                    {"error": "Failed to generate reading", "details": reading_content}
                ),
                500,
            )
...
def generate_writing_prompt():
    ...
    try:
        ...
        return (
            jsonify({"error": "Writing prompt generation failed", "details": str(e)}),
            500,
        )
    ...
    try:
        ...
        if "error" in roleplay_content:
            return (
                jsonify(
                    {
                        "error": "Failed to generate role-play scenario",
                        "details": roleplay_content,
                    }
                ),
                500,
            )
...
def analyze_image():
    ...
    try:
        ...
        if "error" in analysis_content:
            return (
                jsonify(
                    {"error": "Failed to analyze image", "details": analysis_content}
                ),
                500,
            )
        ...
        return (
            jsonify(
                {"activity_type": "image_recognition", "content": analysis_content}
            ),
            200,
        )
...
def chat_with_tutor():
    ...
    try:
        ...
        response = activity_service.generate_general_chat_response(
...
def get_writing_feedback():
    ...
    try:
        ...
        if "error" in feedback:
            return (
                jsonify({"error": "Failed to generate feedback", "details": feedback}),
                500,
            )
...
def save_activity():
    ...
    try:
        ...
        last_activity = (
            Activity.query.filter_by(path_id=path_id)
            .order_by(Activity.order_in_path.desc())
            .first()
        )
        ...
        for activity in activities.items:
            ...
            activity_data = {
                ...
                "completion_data": {
                    ...
                    "time_spent_minutes": (
                        completion_log.time_spent_minutes if completion_log else None
                    ),
                    "attempt_number": (
                        completion_log.attempt_number if completion_log else 0
                    ),
                },
            }
    ...
    try:
        ...
        activity_data = {
            ...
            "user_progress": {
                ...
                "completed_at": (
                    completion_log.completed_at.isoformat() if completion_log else None
                ),
                "score": completion_log.score if completion_log else None,
                "max_score": completion_log.max_score if completion_log else None,
                "time_spent_minutes": (
                ...
                "user_response": (
                    completion_log.user_response if completion_log else None
                ),
                "feedback_provided": (
                    completion_log.feedback_provided if completion_log else None
                ),
            },
        }
    ...
    try:
        ...
        def protected_route():
            ...
            if not data:
                return (
                    jsonify(
                        {"error": "No data provided", "telugu_message": "డేటా అందించబడలేదు"}
                    ),
                    400,
                )
            ...
            if existing_log:
                ...
                existing_log.score = max(
                    existing_log.score or 0, score
                )  # Keep best score
                existing_log.user_response = user_answers
                existing_log.feedback_provided = feedback
                existing_log.time_spent_minutes = (
                    existing_log.time_spent_minutes or 0
                ) + time_spent
                ...
                jsonify(
                    {
                        ...
                        "user_progress": {
                            "attempt_number": (
                                existing_log.attempt_number if existing_log else 1
                            ),
                            "best_score": (
                                max(existing_log.score or 0, score)
                                if existing_log
                                else score
                            ),
                            "total_time_spent": (
                                (existing_log.time_spent_minutes or 0) + time_spent
                                if existing_log
                                else time_spent
                            ),
                        },
...
def get_learning_path_activities(learning_path_id):
    ...
    try:
        ...
        for activity in activities:
            ...
            activity_data = {
                ...
                "progress": {
                    ...
                    "percentage": (
                        round(
                            (completion_log.score / completion_log.max_score * 100), 1
                        )
                        if completion_log and completion_log.max_score > 0
                        else 0
                    ),
        ...
        completed_count = len(
            [a for a in activity_list if a["progress"]["is_completed"]]
        )
            ...
            jsonify(
                {
                    ...
                    "progress_summary": {
                        ...
                        "estimated_total_time": sum(
                            a["estimated_duration_minutes"] for a in activity_list
                        ),
                        "time_spent": sum(
                            log.time_spent_minutes or 0 for log in completed_logs
                        ),
                    },
                }
            ),
...
def get_next_activity():
    ...
    try:
        ...
        for learning_path in user.enrolled_paths:
            # Get activities in order
            activities = (
                Activity.query.filter_by(learning_path_id=learning_path.id)
                .order_by(Activity.order_in_path)
                .all()
            )
...
def get_user_activity_summary():
    ...
    try:
        ...
        if scores_with_max:
            percentages = [
                (score / max_score * 100) for score, max_score in scores_with_max
            ]
        ...
        recent_activity_info = next(
            (a for a in activities if a.id == recent_log.activity_id), None
        )

        recent_activity = (
            {
                "activity_title": (
                    recent_activity_info.title if recent_activity_info else "Unknown"
                ),
                "activity_type": (
                    recent_activity_info.activity_type
                    if recent_activity_info
                    else "Unknown"
                ),
...
def get_all_activities():
    ...
    try:
        ...
        activity_type = request.args.get(
            "activity_type"
        )  # quiz, flashcard, reading, writing, role_play
        ...
        sort_by = request.args.get(
            "sort_by", "created_at"
        )  # created_at, title, difficulty_level, duration
            ...
            activities.append(
                {
                    ...
                    "created_at": activity.created_at.isoformat(),
                    "content_preview": (
                        str(activity.content)[:200] + "..."
                        if len(str(activity.content)) > 200
                        else str(activity.content)
                    ),
                }
            )
    ...
    try:
        ...
        for activity in paginated_activities.items:
            ...
            if activity_type == "quiz" and activity.content:
                activity_data["question_count"] = len(
                    activity.content.get("questions", [])
                )
            elif activity_type == "flashcard" and activity.content:
                activity_data["card_count"] = len(
                    activity.content.get("flashcards", [])
                )
            elif activity_type == "reading" and activity.content:
                activity_data["word_count"] = (
                    len(activity.content.get("passage", "").split())
                    if activity.content.get("passage")
                    else 0
                )
...
def get_my_generated_activities():
    ...
    try:
        ...
        activity_type = request.args.get("activity_type")
        difficulty_level = request.args.get("difficulty_level")
        days_back = request.args.get(
            "days_back", type=int
        )  # Activities from last N days
        page = request.args.get("page", 1, type=int)
        per_page = min(request.args.get("per_page", 20, type=int), 100)

        # Get user's activity logs with activities
        query = (
            db.session.query(Activity, UserActivityLog)
            .join(UserActivityLog, Activity.id == UserActivityLog.activity_id)
        ...
        for activity, log in results:
            activity_data = {
                ...
                "user_progress": {
                    ...
                    "percentage": (
                        round((log.score / log.max_score * 100), 1)
        ...
        completed_activities = len(
            [a for a in my_activities if a["user_progress"]["is_completed"]]
        )
        completion_rate = (
            round((completed_activities / total_activities * 100), 1)
        ...
        # Calculate average score
        scores = [
            a["user_progress"]["percentage"]
            for a in my_activities
            if a["user_progress"]["percentage"] is not None
        ]
        average_score = round(sum(scores) / len(scores), 1) if scores else 0

        # Calculate total time spent
        time_spent = [
            a["user_progress"]["time_spent_minutes"]
            for a in my_activities
            if a["user_progress"]["time_spent_minutes"]
        ]
            ...
            jsonify(
                {
                    ...
                    "data": {"deleted_activity": activity_info},
                }
            ),
            200,
        )
...
def bookmark_activity(activity_id):
    ...
    try:
        ...
        existing_log = UserActivityLog.query.filter_by(
            user_id=current_user_id, activity_id=activity_id
        ).first()
...
def unbookmark_activity(activity_id):
    ...
    try:
        ...
        if (
            not user_log
            or not user_log.user_response
            or not user_log.user_response.get("bookmarked")
        ):
            return (
                jsonify(
                    {
            ...
            jsonify(
                {
                    ...
                    "data": {"activity_id": activity_id, "bookmarked": False},
                }
            ),
            200,
        )
            ...
            bookmarked_activities.append(
                {
                    ...
                    "created_at": activity.created_at.isoformat(),
                    "bookmarked_at": (
                        log.user_response.get("bookmarked_at")
...
def search_activities():
    ...
    try:
        ...
        completion_status = request.args.get(
            "completion_status"
        )  # completed, not_completed, bookmarked
        user_score_min = request.args.get(
            "user_score_min", type=int
        )  # Minimum percentage score
        user_score_max = request.args.get(
            "user_score_max", type=int
        )  # Maximum percentage score
        sort_by = request.args.get(
            "sort_by", "relevance"
        )  # relevance, created_at, title, difficulty, duration, points
        ...
        if completion_status in ["completed", "not_completed", "bookmarked"]:
            if user_score_min:
                query = query.filter(
                    (UserActivityLog.score * 100 / UserActivityLog.max_score)
                    >= user_score_min
        ...
        for result in results:
            if completion_status in ["completed", "not_completed", "bookmarked"]:
                ...
                activity_data = {
                    ...
                    "user_progress": {
                        ...
                        "is_completed": log.is_completed,
                        "is_bookmarked": (
                            log.user_response.get("bookmarked", False)
                            if log.user_response
                            else False
                        ),
                    },
            ...
            if keyword:
                highlighted_title = (
                    activity.title.replace(keyword, f"**{keyword}**")
                    if keyword.lower() in activity.title.lower()
                    else activity.title
                )
