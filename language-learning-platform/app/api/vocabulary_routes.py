...
def update_vocabulary_word(word_id):
    ...
    try:
        ...
        if "difficulty_level" in data and data["difficulty_level"] in [
            "beginner",
            "intermediate",
            "advanced",
        ]:
            word.difficulty_level = data["difficulty_level"]
        if "mastery_level" in data and data["mastery_level"] in [
            "learning",
            "familiar",
            "mastered",
        ]:
            ...
            jsonify(
                {
                    ...
                    "word": {
                        ...
                        "correct_count": word.correct_count,
                        "accuracy": (
                            word.correct_count / word.practice_count
...
def get_vocabulary_stats():
    ...
    try:
        ...
        learning_words = VocabularyWord.query.filter_by(
            user_id=user_id, mastery_level="learning"
        ).count()
        familiar_words = VocabularyWord.query.filter_by(
            user_id=user_id, mastery_level="familiar"
        ).count()
        mastered_words = VocabularyWord.query.filter_by(
            user_id=user_id, mastery_level="mastered"
        ).count()

        # Get difficulty distribution
        beginner_words = VocabularyWord.query.filter_by(
            user_id=user_id, difficulty_level="beginner"
        ).count()
        intermediate_words = VocabularyWord.query.filter_by(
            user_id=user_id, difficulty_level="intermediate"
        ).count()
        advanced_words = VocabularyWord.query.filter_by(
            user_id=user_id, difficulty_level="advanced"
        ).count()
            ...
            jsonify(
                {
                    ...
                    "stats": {
                        ...
                        "mastery_percentage": {
                            "learning": (
                                (learning_words / total_words * 100)
                                if total_words > 0
                                else 0
                            ),
                            "familiar": (
                                (familiar_words / total_words * 100)
                                if total_words > 0
                                else 0
                            ),
                            "mastered": (
...
def generate_practice_flashcards():
    ...
    try:
        ...
        # Get words
        words = (
            query.order_by(VocabularyWord.last_practiced.asc().nullsfirst())
            .limit(num_cards)
            .all()
        )
            ...
            flashcards.append(
                {
                    ...
                    "definition": word.definition
                    or f"The word '{word.english_word}' in English",
                    "example": word.example_sentence
                    or f"I learned the word '{word.english_word}' today.",
