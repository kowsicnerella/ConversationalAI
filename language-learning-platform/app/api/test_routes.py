...
def start_test(test_id):
    ...
    try:
        ...
        test = TestAssessment.query.filter_by(id=test_id, user_id=user_id).first()

        if not test:
            return (
                jsonify(
                    {"error": "Test not found", "telugu_message": "పరీక్ష కనుగొనబడలేదు"}
                ),
                404,
            )
...
def _generate_comprehensive_test_questions(
    user_id, chapters, num_questions, question_types, test_type
):
    """
    ...
    try:
        ...
        learning_context = practice_agent._analyze_user_learning_context(
            user_id, chapters[0].id
        )
    ...
    for skill, performance in skill_analysis.items():
        if performance["score_percentage"] < 60:
            recommendations.append(
                f"Focus on improving {skill} skills through targeted practice."
            )
    ...
    try:
        for chapter_id in test.chapter_ids:
            ...
            if progress:
                ...
                chapter_performance = test_results["detailed_analysis"][
                    "chapter_performance"
                ].get(str(chapter_id), {})
...
def _get_question_breakdown(test):
    ...
    answer_lookup = {
        answer["question_id"]: answer for answer in test.user_responses or []
    }
        ...
        breakdown.append(
            {
                ...
                "correct_answer": question.get("correct_answer", ""),
                "is_correct": _evaluate_test_answer(
                    question, user_response.get("user_answer", "")
                ),
                "explanation": question.get("explanation", ""),
                "skill_tested": question.get("skill_tested", ""),
                "difficulty": question.get("difficulty", ""),
