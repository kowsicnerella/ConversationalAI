...
def set_user_goals():
    ...
    try:
        ...
        goal = personalization_service.create_user_goal(
            user_id, daily_time_goal, learning_focus
        )
        ...
        if "preferred_topics" in data:
            ...
            if isinstance(preferred_topics, list) and len(preferred_topics) > 0:
                user.learning_goals = (
                    ...
                    if not isinstance(user.learning_goals, str)
                    else user.learning_goals
                )
...
def get_user_preferences():
    ...
    try:
        ...
        preferences = {
            ...
            "learning_goal_type": (
                user.get_learning_goals()[0]
                if user.get_learning_goals()
                else "general_english"
            ),
            "notification_settings": user.get_notification_preferences()
            or {
                "daily_reminders": True,
                "streak_alerts": True,
                "achievement_notifications": True,
...
def start_proficiency_assessment():
    ...
    try:
        ...
        assessment_data = personalization_service.conduct_proficiency_assessment(
            user_id
        )
...
def respond_to_assessment(assessment_id):
    ...
    try:
        ...
        if "error" in result:
            return (
                jsonify(
                    {"error": result["error"], "telugu_message": "సమాధానం మూల్యాంకనంలో లోపం"}
                ),
                400,
            )
...
def start_session():
    ...
    try:
        ...
        session_data = personalization_service.start_learning_session(
...
def end_session(session_id):
    ...
    try:
        ...
        session_summary = personalization_service.end_learning_session(
            session_id, user_satisfaction
        )
...
def track_vocabulary():
    ...
    try:
        ...
        if "error" in vocab_data:
            return (
                jsonify(
                    {"error": vocab_data["error"], "telugu_message": "పదజాలం ట్రాకింగ్‌లో లోపం"}
                ),
                400,
            )
...
def practice_vocabulary_word(vocab_id):
    ...
    try:
        ...
        vocab_word = VocabularyWord.query.filter_by(
            id=vocab_id, user_id=user_id
        ).first()
        if not vocab_word:
            return (
                jsonify(
