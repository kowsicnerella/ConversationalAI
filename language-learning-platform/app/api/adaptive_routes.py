            ...
            jsonify(
                {
                    ...
                    "recommendations": {
                        "overall_level": _get_performance_level(
                    ...
                    "analysis_timestamp": performance.get(
            ...
            jsonify(
                {
                    ...
                    "estimated_improvement_timeline": _calculate_improvement_timeline(
                        gaps_analysis
                    ),
                }
            ),
            ...
            jsonify(
                {
                    ...
                    "implementation_guide": _generate_pace_implementation_guide(
                        pace_analysis
                    ),
    ...
    try:
        ...
        profile = {
            "user_info": {
                ...
                "total_activities_completed": UserActivityLog.query.filter_by(
                    user_id=user_id
                ).count(),
            ...
            "learning_gaps_summary": {
                "priority_areas": [
                    gap["skill"] for gap in gaps.get("prioritized_gaps", [])[:3]
                ],
                "improvement_areas": len(
                    [
                        gap
                        for gap in gaps.get("prioritized_gaps", [])
                        if gap.get("gap_data", {}).get("gap_level") == "high"
                    ]
                ),
            ...
            "adaptive_insights": {
                ...
                "motivation_factors": _identify_motivation_factors(
                    user_id, performance
                ),
...
def _identify_improvement_areas(performance: Dict) -> List[str]:
    ...
    for activity_type, score in performance.get(
...
def _generate_telugu_performance_summary(performance: Dict) -> str:
    ...
    if accuracy >= 0.8:
        summary = (
            f"మీ పనితీరు అద్భుతంగా ఉంది! {total_activities} కార్యకలాపాలలో {accuracy:.1%} ఖచ్చితత్వం"
        )
    elif accuracy >= 0.6:
...
def _generate_telugu_gap_insights(gaps_analysis: Dict) -> List[str]:
    ...
    if (
        gaps_analysis.get("overall_assessment", {}).get("overall_level")
        == "needs_attention"
    ):
...
def _calculate_success_predictors(performance: Dict, gaps: Dict) -> Dict:
    ...
    success_score = (
        ...
        + min(total_activities / 100, 1) * 0.2
    )
    ...
    return {
        ...
        "improvement_potential": (
            ...
            else "medium" if success_score > 0.4 else "needs_support"
        ),
    }
