...
def get_dashboard_summary():
    ...
    try:
        ...
        monthly_time = (
            ...
            .filter(
                LearningSession.user_id == user_id,
                LearningSession.start_time >= month_ago,
            )
            .scalar()
            or 0
        )
        ...
        weekly_activities = UserActivityLog.query.filter(
            UserActivityLog.user_id == user_id, UserActivityLog.completed_at >= week_ago
        ).count()

        # Vocabulary analytics
        total_vocabulary = VocabularyWord.query.filter_by(user_id=user_id).count()
        mastered_words = VocabularyWord.query.filter(
            VocabularyWord.user_id == user_id, VocabularyWord.mastery_level >= 0.8
        ).count()

        weekly_new_words = VocabularyWord.query.filter(
            VocabularyWord.user_id == user_id, VocabularyWord.discovered_at >= week_ago
        ).count()
        ...
        if current_goal and current_goal.daily_time_goal_minutes > 0:
            goal_progress = min(
                100, (today_time / current_goal.daily_time_goal_minutes) * 100
            )
        ...
        summary = {
            ...
            "activities": {
                ...
                "improvement": (
                    round(recent_avg_score - avg_score, 1) if avg_score > 0 else 0
                ),
            },
            "vocabulary": {
                ...
                "mastery_rate": (
                    round((mastered_words / total_vocabulary * 100), 1)
            ...
            "streaks_and_goals": {
                ...
                "daily_goal_minutes": (
                    current_goal.daily_time_goal_minutes if current_goal else 0
                ),
            },
        }
...
def get_learning_trends():
    ...
    try:
        ...
        daily_sessions = (
            ...
            .group_by(func.date(LearningSession.start_time))
            .all()
        )
        ...
        daily_activities = (
            ...
            .group_by(func.date(UserActivityLog.completed_at))
            .all()
        )
        ...
        daily_vocabulary = (
            ...
            .group_by(func.date(VocabularyWord.discovered_at))
            .all()
        )
        ...
        for activity in daily_activities:
            ...
            if date_key in daily_data:
                daily_data[date_key]["activities_completed"] = (
                    activity.activities_completed or 0
                )
                daily_data[date_key]["average_score"] = round(
                    activity.avg_score or 0, 1
                )
        ...
        if days >= 14:
            ...
            prev_week_time = (
                sum(day["learning_time_minutes"] for day in prev_week)
                if prev_week
                else 0
            )
            ...
            time_trend = (
                "increasing"
                if last_week_time > prev_week_time
                else "decreasing" if last_week_time < prev_week_time else "stable"
            )
        else:
            time_trend = "stable"
        ...
        insights = {
            ...
            "most_productive_day": (
                max(daily_values, key=lambda x: x["learning_time_minutes"])["date"]
                if daily_values
                else None
            ),
            "consistency_score": len(
                [day for day in daily_values if day["learning_time_minutes"] > 0]
            )
            / days
            * 100,
...
def get_performance_analysis():
    ...
    try:
        ...
        activity_performance = (
            ...
            .join(Activity, UserActivityLog.activity_id == Activity.id)
            .filter(UserActivityLog.user_id == user_id)
        ...
        difficulty_performance = (
            ...
            .join(Activity, UserActivityLog.activity_id == Activity.id)
            .filter(UserActivityLog.user_id == user_id)
            .group_by(Activity.difficulty_level)
            .all()
        )
        ...
        vocab_mastery = (
            ...
            .group_by(VocabularyWord.mastery_level)
            .all()
        )

        # Strengths and improvement areas
        activity_scores = {
            perf.activity_type: perf.avg_score for perf in activity_performance
        }
        strengths = [
            activity for activity, score in activity_scores.items() if score >= 80
        ]
        improvement_areas = [
            activity for activity, score in activity_scores.items() if score < 70
        ]
        ...
        performance_data = {
            "by_activity_type": [
                {
                    ...
                    "performance_level": (
                        ...
                        else "good" if perf.avg_score >= 70 else "needs_improvement"
                    ),
                }
            ...
            "analysis": {
                ...
                "overall_score": (
                    round(
                        sum(perf.avg_score for perf in activity_performance)
                        / len(activity_performance),
                        1,
                    )
                    if activity_performance
                    else 0
                ),
                "most_practiced_activity": (
                    max(
                        activity_performance, key=lambda x: x.total_attempts
                    ).activity_type
                    if activity_performance
                    else None
                ),
        ...
        if improvement_areas:
            recommendations.append(
                f"Focus more practice on: {', '.join(improvement_areas)}"
            )
        if strengths:
            recommendations.append(
                f"Great job with: {', '.join(strengths)}! Keep it up!"
            )
...
def get_vocabulary_analytics():
    ...
    try:
        ...
        new_words = VocabularyWord.query.filter(
            VocabularyWord.user_id == user_id, VocabularyWord.mastery_level < 0.3
        ).count()
        ...
        words_by_source = (
            ...
            .filter(VocabularyWord.user_id == user_id)
            .group_by(VocabularyWord.source_activity_type)
            .all()
        )
        ...
        top_practiced_words = (
            ...
            .order_by(VocabularyWord.times_practiced.desc())
            .limit(10)
            .all()
        )
        ...
        vocabulary_analytics = {
            "overview": {
                ...
                "new_words": new_words,
                "mastery_rate": (
                    round((mastered_words / total_words * 100), 1)
                    if total_words > 0
                    else 0
                ),
            },
            "by_source": [
                {"source": source.source_activity_type, "word_count": source.word_count}
            ...
            "top_practiced_words": [
                {
                    ...
                    "times_practiced": word.times_practiced,
                    "success_rate": (
                        round((word.times_correct / word.times_practiced * 100), 1)
                        if word.times_practiced > 0
                        else 0
                    ),
                    "mastery_level": word.mastery_level,
                }
...
def export_progress_report():
    ...
    try:
        ...
        report_type = request.args.get(
            "type", "summary"
        )  # 'summary', 'detailed', 'vocabulary'

        user = User.query.get(user_id)
        if not user:
            return (
                jsonify(
                    {"error": "User not found", "telugu_message": "వినియోగదారు కనుగొనబడలేదు"}
                ),
                404,
            )
        ...
        if report_type == "summary":
            ...
            report = {
                ...
                "summary": {
                    ...
                    "current_streak": (
                        user.profile.current_streak if user.profile else 0
                    ),
                },
            }
            ...
            report = {
                ...
                "vocabulary_summary": {
                    ...
                    "learning": len(
                        [w for w in vocab_words if 0.3 <= w.mastery_level < 0.8]
                    ),
...
def get_activity_performance_analysis():
    ...
    try:
        ...
        # Build base query
        query = UserActivityLog.query.filter_by(user_id=user_id).filter(
        ...
        for level, data in difficulty_performance.items():
            level_logs = [
                log
                for log in logs
                if activity_dict.get(log.activity_id)
                and activity_dict[log.activity_id].difficulty_level == level
            ]
            if level_logs:
                ...
                data["avg_time"] = sum(
                    log.time_spent_minutes or 0 for log in level_logs
                ) / len(level_logs)
...
def analyze_learning_patterns():
    ...
    try:
        ...
        for log in recent_logs:
            ...
            week_start = (
                log.completed_at.date() - timedelta(days=log.completed_at.weekday())
            ).isoformat()
            if week_start not in weekly_progress:
                weekly_progress[week_start] = {
                    "activities": 0,
                    "total_score": 0,
                    "total_max": 0,
                }
            ...
            if log.score and log.max_score and log.max_score > 0:
                ...
                if accuracy < 0.7:  # Less than 70% accuracy
                    mistake_patterns.append(
                        {
                            "activity_type": (
                                activity.activity_type if activity else "unknown"
                            ),
                            "accuracy": accuracy,
                            "date": log.completed_at.date().isoformat(),
        ...
        avg_activities_per_active_day = (
            sum(daily_activity_counts.values()) / active_days if active_days > 0 else 0
        )
        ...
        for week, data in sorted(weekly_progress.items()):
            ...
            weekly_trends.append(
                {"week": week, "activities": data["activities"], "accuracy": accuracy}
            )
            ...
            jsonify(
                {
                    ...
                    "learning_metrics": {
                        ...
                        "avg_activities_per_day": round(
                            avg_activities_per_active_day, 1
                        ),
    ...
    try:
        ...
        for period_name, start_date in periods.items():
            ...
            completed_activities = len(
                [
                    ...
                    if log.score
                    and log.max_score
                    and (log.score / log.max_score) >= 0.7
                ]
            )
            completion_rate = (
                (completed_activities / total_sessions * 100)
                if total_sessions > 0
                else 0
            )
            ...
            session_lengths = [
                log.time_spent_minutes or 0 for log in logs if log.time_spent_minutes
            ]
            avg_session_length = (
        ...
        engagement_score = min(
            ...
            (
                ...
                + (
                    min(recent_data["total_sessions"], 60) / 60 * 10
                )  # 10% for activity volume
            ),
        )
            ...
            jsonify(
                {
                    ...
                    "data_confidence": (
                        "high"
                        if total_logs >= 20
                        else "medium" if total_logs >= 10 else "low"
                    ),
                }
            ),
            200,
