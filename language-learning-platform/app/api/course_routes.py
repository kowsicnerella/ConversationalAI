...
def get_learning_paths():
    ...
    try:
        ...
        enrolled_path_ids = (
            [path.id for path in user.enrolled_paths] if user.enrolled_paths else []
        )
        ...
        for path in paths.items:
            ...
            completed_activities = (
                ...
                if path.id in enrolled_path_ids
                else 0
            )
...
def get_learning_path_details(path_id):
    ...
    try:
        ...
        activities = (
            Activity.query.filter_by(learning_path_id=path_id)
        ...
        if is_enrolled:
            completed_logs = UserActivityLog.query.filter_by(
                user_id=user_id, learning_path_id=path_id
            ).all()
        ...
        for activity in activities:
            ...
            if activity.id in completed_activity_ids:
                completion_log = next(
                    log for log in completed_logs if log.activity_id == activity.id
                )
            ...
            jsonify(
                {
                    ...
                    "learning_path": {
                        ...
                        "progress": {
                            ...
                            "completion_percentage": round(completion_percentage, 1),
                            "next_activity": next(
                                (a for a in activity_list if not a["is_completed"]),
                                None,
                            ),
                        },
...
def enroll_in_learning_path(path_id):
    ...
    try:
        ...
        if path.prerequisites:
            ...
            if (
                required_level
                and required_level not in ["beginner"]
                and user_proficiency == "beginner"
            ):
                return (
                    jsonify(
                        {
...
def get_enrollment_progress(path_id):
    ...
    try:
        ...
        total_time_spent = sum(
            log.time_spent_minutes for log in completed_logs if log.time_spent_minutes
        )
...
def get_my_learning_paths():
    ...
    try:
        ...
        for path in user.enrolled_paths:
            ...
            # Get enrollment info
            enrollment_info = (
                user.enrollment_data.get(str(path.id), {})
                if user.enrollment_data
                else {}
            )
            ...
            next_activity = (
                Activity.query.filter_by(learning_path_id=path.id)
            ...
            time_spent = (
                ...
                .filter(
                    ...
                    UserActivityLog.learning_path_id == path.id,
                )
                .scalar()
                or 0
            )
            ...
            enrolled_paths.append(
                {
                    ...
                    "enrollment": {
                        ...
                        "last_activity_at": (
                            ...
                            if UserActivityLog.query.filter_by(
                                user_id=user_id, learning_path_id=path.id
                            ).first()
                            else None
                        ),
        ...
        try:
            if activity.activity_type == "quiz":
                activity_content = activity_service.generate_quiz(
                    topic=activity.title, level=activity.difficulty_level
                )
                ...
                activity_content = activity_service.generate_role_playing_scenario(
                    scenario_type=activity.title, level=activity.difficulty_level
                )
            else:
            ...
            current_app.logger.warning(
                f"Content generation failed: {str(content_error)}"
            )
...
def complete_activity(activity_id):
    ...
    try:
        ...
        next_activity = (
            Activity.query.filter_by(learning_path_id=activity.learning_path_id)
        ...
        completed_activities = UserActivityLog.query.filter_by(
            user_id=user_id, learning_path_id=activity.learning_path_id
        ).count()
            ...
            jsonify(
                {
                    ...
                    "completion": {
                        ...
                        "performance_message": (
                            "Excellent work!"
                            if score >= 80
                            else "Good job!" if score >= 60 else "Keep practicing!"
                        ),
                    },
                    "progress": {
                    ...
                    "celebration": {
                        ...
                        "message": (
                            "Great job! You're making excellent progress! గొప్పగా చేశారు!"
                            if score >= 80
                            else None
                        ),
                    },
