...
def analyze_uploaded_image():
    ...
    try:
        ...
        is_camera_capture = (
            request.form.get("is_camera_capture", "false").lower() == "true"
        )
        ...
        if not is_valid:
            return (
                jsonify(
                    {"success": False, "error": error_msg, "error_telugu": "చిత్రం చెల్లదు"}
                ),
                400,
            )

        # Save image
        success, file_path, filename, save_error = ImageService.save_uploaded_image(
            file, user_id
        )
        if not success:
            return (
                ...
                jsonify(
                    {
                        ...
                        "scene_description": analysis_result.get(
                            "scene_description", ""
                        ),
                        "scene_description_telugu": analysis_result.get(
                            "scene_description_telugu", ""
                        ),
                        "learning_context": analysis_result.get("learning_context", ""),
                        "total_objects_found": analysis_result.get(
                ...
                jsonify(
                    {
                        ...
                        "error_telugu": analysis_result.get(
                            "error_telugu", "విశ్లేషణ విఫలమైంది"
                        ),
                        "image_id": image_record.id,
...
def get_image_details(image_id):
    ...
    try:
        ...
        return jsonify({"success": True, "image": image_record.to_dict()}), 200
...
def save_words_to_vocabulary(image_id):
    ...
    try:
        ...
        result = ImageService.save_objects_to_vocabulary(
            image_id, user_id, object_indices
        )
        return jsonify(result), 200 if result.get("success") else 500
