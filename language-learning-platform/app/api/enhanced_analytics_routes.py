...
def get_performance_trends():
    ...
    try:
        ...
        return (
            jsonify({"error": "Failed to get performance trends", "details": str(e)}),
            500,
        )
...
def get_learning_streaks():
    ...
    try:
        ...
        streaks = LearningStreak.query.filter(
            LearningStreak.user_id == user_id, LearningStreak.is_active == True
        ).all()
        ...
        return jsonify({"success": True, "streaks": streak_data}), 200

    except Exception as e:
        return (
            jsonify({"error": "Failed to get learning streaks", "details": str(e)}),
            500,
        )
...
def get_skill_breakdown():
    ...
    try:
        ...
        recent_assessments = (
            ProficiencyAssessment.query.filter(
                ProficiencyAssessment.user_id == user_id,
                ProficiencyAssessment.status == "completed",
            )
            .order_by(desc(ProficiencyAssessment.completed_at))
            .limit(5)
            .all()
        )
        ...
        skill_analytics = (
            ...
            .group_by(UserAnalytics.skill_area)
            .all()
        )
        ...
        question_performance = (
            db.session.query(
                ...
                func.avg(
                    func.cast(AssessmentQuestionResponse.is_correct, db.Float)
                ).label("accuracy"),
                func.avg(AssessmentQuestionResponse.time_spent_seconds).label(
                    "avg_time"
                ),
                func.count(AssessmentQuestionResponse.id).label("question_count"),
            )
            .filter(AssessmentQuestionResponse.user_id == user_id)
            .group_by(AssessmentQuestionResponse.skill_area)
            .all()
        )
        ...
        for skill, accuracy, avg_time, q_count in question_performance:
            if skill:
                ...
                skill_breakdown[skill]["accuracy"] = (
                    float(accuracy) if accuracy else 0.0
                )
                skill_breakdown[skill]["average_time_seconds"] = (
                    float(avg_time) if avg_time else 0.0
                )
        ...
        return (
            jsonify({"error": "Failed to get skill breakdown", "details": str(e)}),
            500,
        )
...
def get_time_analytics():
    ...
    try:
            ...
            jsonify(
                {
                    ...
                    "daily_breakdown": {
                        date: dict(activities)
                        for date, activities in daily_breakdown.items()
                    },
                    "date_range": {
                        "start": start_date.isoformat(),
                        "end": end_date.isoformat(),
        ...
        return (
            jsonify({"error": "Failed to get time analytics", "details": str(e)}),
            500,
        )
...
def get_difficulty_progression():
    ...
    try:
        ...
        difficulty_stats = (
            ...
            .group_by(AssessmentQuestionResponse.difficulty_level)
            .all()
        )

        # Also get from activity responses
        activity_difficulty_stats = (
            db.session.query(
                ActivityQuestionResponse.difficulty_level,
                func.avg(
                    func.cast(ActivityQuestionResponse.is_correct, db.Float)
                ).label("accuracy"),
            ...
            .group_by(ActivityQuestionResponse.difficulty_level)
            .all()
        )
        ...
        for difficulty, accuracy, avg_time, count in activity_difficulty_stats:
            if difficulty in combined_stats:
                ...
                combined_stats[difficulty]["accuracy"] = (
                    combined_stats[difficulty]["accuracy"]
                    * combined_stats[difficulty]["question_count"]
                    + float(accuracy if accuracy else 0.0) * count
                ) / total_questions
                combined_stats[difficulty]["average_time_seconds"] = (
                    combined_stats[difficulty]["average_time_seconds"]
                    * combined_stats[difficulty]["question_count"]
                    + float(avg_time if avg_time else 0.0) * count
                ) / total_questions
        ...
        return jsonify({"success": True, "difficulty_progression": progression}), 200

    except Exception as e:
        return (
            jsonify(
                {"error": "Failed to get difficulty progression", "details": str(e)}
            ),
            500,
        )
...
def get_learning_timeline():
    ...
    try:
        ...
        timeline_events = (
            query.order_by(desc(UserLearningTimeline.created_at)).limit(100).all()
        )
        ...
        return (
            jsonify({"error": "Failed to get learning timeline", "details": str(e)}),
            500,
        )
...
def get_comprehensive_report():
    ...
    try:
        ...
        avg_assessment_score = (
            db.session.query(
                func.avg(
                    ProficiencyAssessment.score
                    * 100.0
                    / ProficiencyAssessment.max_score
                )
            )
            .filter(
                ProficiencyAssessment.user_id == user_id,
        ...
        # Time spent
        total_time = (
            db.session.query(func.sum(UserActivityLog.time_spent_minutes))
            .filter(
                UserActivityLog.user_id == user_id,
                func.date(UserActivityLog.completed_at) >= start_date,
            )
        ...
        # Recent improvements
        recent_timeline = (
            UserLearningTimeline.query.filter(
                UserLearningTimeline.user_id == user_id,
                UserLearningTimeline.created_at >= datetime.now() - timedelta(days=7),
            )
            .order_by(desc(UserLearningTimeline.created_at))
            .limit(10)
            .all()
        )
        ...
        return (
            jsonify(
                {"error": "Failed to generate comprehensive report", "details": str(e)}
            ),
            500,
        )
