...
# Create blueprint
gamification_bp = Blueprint('gamification_phase9', __name__, url_prefix='/api/gamification-v2')
...
def get_challenge_history():
    ...
    try:
        ...
        challenges = GamificationChallenge.query.filter(
            ...
            GamificationChallenge.challenge_date >= date.today() - timedelta(days=30)
        ).order_by(GamificationChallenge.challenge_date.desc()).all()
...
def manual_complete_challenge(challenge_id):
    ...
    try:
        ...
        challenge = GamificationChallenge.query.get(challenge_id)
...
def get_achievements():
    ...
    Returns:
        200: GamificationAchievement data
        500: Server error
    """
    try:
        user_id = get_jwt_identity()
...
def showcase_achievement(achievement_id):
    ...
    Args:
        achievement_id: GamificationAchievement ID
        
    Returns:
        200: Showcase updated
        404: GamificationAchievement not found
        500: Server error
    """
    try:
        user_id = get_jwt_identity()
...
def share_achievement():
    ...
    Returns:
        201: GamificationAchievement shared
        404: GamificationAchievement not found
        500: Server error
    """
    try:
