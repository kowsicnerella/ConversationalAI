...
class Activity(db.Model):
    ...
    )  # quiz, flashcard, reading, writing, role_play, image_recognition
    ...
    is_adaptive = db.Column(
        db.Boolean, default=False
    )  # Whether this is an adaptive activity
    prerequisite_concepts = db.Column(db.JSON)  # Required concepts to master first
    mastery_threshold = db.Column(
        db.Float, default=0.8
    )  # Threshold for concept mastery
    is_validation = db.Column(
        db.Boolean, default=False
    )  # Whether this validates mastery
    is_retry = db.Column(db.Boolean, default=False)  # Whether this is a retry activity
    is_reinforcement = db.Column(
        db.Boolean, default=False
    )  # Whether this reinforces learning
    parent_activity_id = db.Column(
        db.Integer, db.ForeignKey("activities.id")
    )  # For retry/reinforcement activities

    # AI-generated metadata
    generation_metadata = db.Column(
        db.JSON
    )  # Stores AI generation context and parameters
    adaptation_history = db.Column(db.JSON)  # History of how activity has been adapted

    # Relationships
    user_logs = db.relationship(
        "UserActivityLog",
        backref="activity",
        lazy="dynamic",
        cascade="all, delete-orphan",
    )
    child_activities = db.relationship(
        "Activity", backref=db.backref("parent_activity", remote_side=[id])
    )
...
class UserActivityLog(db.Model):
    ...
    learning_path_id = db.Column(
        db.Integer, db.ForeignKey("learning_paths.id"), nullable=True
    )  # For easier queries
    ...
    interventions_triggered = db.Column(
        db.JSON
    )  # Interventions provided during activity
...
class ConceptMastery(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("user_id", "skill_area", "concept", "proficiency_level"),
    )

    def __repr__(self):
        return (
            f"<ConceptMastery {self.user.username} - {self.skill_area}:{self.concept}>"
        )
...
class AdaptiveLearningSession(db.Model):
    ...
    completion_reason = db.Column(
        db.String(50)
    )  # natural, break_suggested, time_limit, etc.
...
class AdaptiveLearningPathProgress(db.Model):
    ...
    # Path progression
    current_skill_focus = db.Column(
        db.String(50)
    )  # Current primary skill being worked on
