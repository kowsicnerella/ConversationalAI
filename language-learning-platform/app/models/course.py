...
class LearningPath(db.Model):
    ...
    )  # List of prerequisite learning path IDs or skills
    ...
    )  # Whether this is an adaptive path
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"))  # For personalized paths
    assessment_id = db.Column(
        db.Integer, db.ForeignKey("proficiency_assessments.id")
    )  # Source assessment
    ...
    created_by_user = db.relationship(
        "User", foreign_keys=[user_id], backref="created_learning_paths"
    )
    source_assessment = db.relationship(
        "ProficiencyAssessment", backref="generated_learning_paths"
    )
