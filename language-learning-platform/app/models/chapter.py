...
class UserChapterProgress(db.Model):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "chapter_id", name="unique_user_chapter_progress"),
    )
...
class UserNotes(db.Model):
    ...
    practice_session_id = db.Column(
        db.Integer, db.ForeignKey("practice_sessions.id"), nullable=True
    )

    note_content = db.Column(db.Text, nullable=False)
    note_type = db.Column(
        db.String(50), default="general"
    )  # general, vocabulary, grammar, mistake
...
class TestAssessment(db.Model):
    ...
    )  # chapter_test, comprehensive_test, placement_test
...
class ChapterDependency(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    chapter_id = db.Column(db.Integer, db.ForeignKey("chapters.id"), nullable=False)
    prerequisite_chapter_id = db.Column(
        db.Integer, db.ForeignKey("chapters.id"), nullable=False
    )
    is_strict = db.Column(
        db.Boolean, default=True
    )  # Must complete prerequisite vs recommended

    # Unique constraint to prevent duplicate dependencies
    __table_args__ = (
        UniqueConstraint(
            "chapter_id", "prerequisite_chapter_id", name="unique_chapter_dependency"
        ),
    )
...
class AIConversationContext(db.Model):
    ...
    context_type = db.Column(
        db.String(50), default="practice_assistance"
    )  # practice_assistance, general_help, explanation
    ...
    user_learning_state = db.Column(
        db.JSON
    )  # User's current learning state for context
