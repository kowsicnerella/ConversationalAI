...
class ImageLearning(db.Model):
    ...
    objects_saved_to_vocabulary = db.Column(db.JSON, default=list)  # List of saved object IDs
    flashcard_session_created = db.Column(db.Boolean, default=False)
    flashcard_session_id = db.Column(db.Integer, db.ForeignKey('learning_sessions.id'), nullable=True)
    
    # Device/source information
    uploaded_from_device = db.Column(db.String(50))  # mobile, desktop, tablet
    is_camera_capture = db.Column(db.Boolean, default=False)  # True if from camera, False if file upload
    ...
    # Relationships
    user = db.relationship('User', backref='image_learning_sessions', foreign_keys=[user_id])
    flashcard_session = db.relationship('LearningSession', backref='source_image', foreign_keys=[flashcard_session_id])
...
class ImageObjectVocabulary(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    image_learning_id = db.Column(db.Integer, db.ForeignKey('image_learning.id'), nullable=False)
    vocabulary_word_id = db.Column(db.Integer, db.ForeignKey('vocabulary_words.id'), nullable=False)
