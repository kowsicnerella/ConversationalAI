...
class AssessmentQuestionResponse(db.Model):
    ...
    assessment_id = db.Column(
        db.Integer, db.ForeignKey("proficiency_assessments.id"), nullable=False
    )
...
class UserAnalytics(db.Model):
    ...
    metric_type = db.Column(
        db.String(50), nullable=False
    )  # accuracy, speed, consistency, improvement_rate
    ...
    )  # Number of sessions included in this metric
...
class LearningStreak(db.Model):
    ...
    )  # daily, weekly, activity_specific, skill_specific
...
class AIGeneratedContent(db.Model):
    ...
    content_type = db.Column(
        db.String(50), nullable=False
    )  # question, feedback, explanation, activity
    ...
    )  # Manual approval for quality content
...
class UserLearningTimeline(db.Model):
    ...
    event_type = db.Column(
        db.String(50), nullable=False
    )  # activity_completed, assessment_taken, badge_earned, streak_achieved
    event_subtype = db.Column(
        db.String(50)
    )  # specific type like quiz_completed, vocabulary_mastered
    event_data = db.Column(db.JSON)  # Detailed event information
    related_id = db.Column(
        db.Integer
    )  # ID of related record (activity_id, assessment_id, etc.)
...
class PerformanceTrend(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "user_id", "trend_period", "period_start", name="unique_user_trend_period"
        ),
    )
