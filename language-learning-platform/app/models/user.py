...
class User(db.Model):
    ...
    )  # Increased from 128 to 255 for modern hash algorithms
    ...
    enrollment_data = db.Column(
        db.JSON, default=dict
    )  # JSON for enrollment tracking data

    # Onboarding and learning workflow tracking
    onboarding_completed = db.Column(
        db.Boolean, default=False
    )  # Has user completed onboarding?
    needs_initial_assessment = db.Column(
        db.Boolean, default=True
    )  # Does user need to take initial assessment?
    assessment_taken_at = db.Column(db.DateTime)  # When was the assessment taken?
    current_learning_phase = db.Column(
        db.String(20), default="onboarding"
    )  # onboarding, assessment, learning, mastery
    ...
    activity_logs = db.relationship(
        "UserActivityLog", backref="user", lazy="dynamic", cascade="all, delete-orphan"
    )
    ...
    enrolled_paths = db.relationship(
        "LearningPath",
    ...
    def set_notification_preferences(self, preferences_dict):
        ...
        self.notification_preferences = (
            json.dumps(preferences_dict) if preferences_dict else None
        )

    def get_notification_preferences(self):
        """Get notification preferences as Python dict"""
        return (
            json.loads(self.notification_preferences)
