...
class UserEnrollment(db.Model):
    ...
    # Progress tracking
    current_chapter_id = db.Column(db.Integer, db.ForeignKey('chapters.id'))  # Current chapter user is on
    total_chapters = db.Column(db.Integer, default=0)
    completed_chapters = db.Column(db.Integer, default=0)
    completion_percentage = db.Column(db.Float, default=0.0)  # Overall path completion %
    ...
    # Performance metrics
    average_score = db.Column(db.Float, default=0.0)  # Average score across all completed activities
    ...
    badges_earned = db.Column(db.JSON, default=list)  # List of badge IDs earned in this path
    certificate_issued = db.Column(db.Boolean, default=False)
    certificate_url = db.Column(db.String(500))  # URL to certificate if completed
    
    # Adaptive learning
    difficulty_adjustments = db.Column(db.JSON, default=list)  # History of difficulty changes
    ...
    __table_args__ = (UniqueConstraint('user_id', 'learning_path_id', name='unique_user_learning_path'),)
    ...
    chapter_progress = db.relationship('ChapterProgress', backref='enrollment', lazy='dynamic', cascade='all, delete-orphan')
    ...
    def to_dict(self):
        ...
        return {
            ...
            'average_score': round(self.average_score, 2) if self.average_score else 0.0,
...
class ChapterProgress(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    enrollment_id = db.Column(db.Integer, db.ForeignKey('user_enrollments.id'), nullable=False)
    ...
    total_activities = db.Column(db.Integer, default=0)
    completed_activities = db.Column(db.Integer, default=0)
    current_activity_index = db.Column(db.Integer, default=0)  # Index of current activity
    ...
    __table_args__ = (UniqueConstraint('enrollment_id', 'chapter_id', name='unique_enrollment_chapter'),)
    
    # Relationships
    chapter = db.relationship('Chapter', backref='user_progress_records')
    activity_progress = db.relationship('ActivityProgress', backref='chapter_progress', lazy='dynamic', cascade='all, delete-orphan')
...
class ActivityProgress(db.Model):
    ...
    learning_session_id = db.Column(db.Integer, db.ForeignKey('learning_sessions.id'), nullable=False)
    
    # Activity details
    activity_type = db.Column(db.String(50), nullable=False)  # quiz, flashcard, reading, writing, roleplay
    activity_index = db.Column(db.Integer, nullable=False)  # Order within chapter (0, 1, 2, ...)
    ...
    __table_args__ = (UniqueConstraint('chapter_progress_id', 'learning_session_id', name='unique_chapter_activity'),)
    
    # Relationships
    learning_session = db.relationship('LearningSession', backref='activity_progress_record')
...
class PathCertificate(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    enrollment_id = db.Column(db.Integer, db.ForeignKey('user_enrollments.id'), nullable=False, unique=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    learning_path_id = db.Column(db.Integer, db.ForeignKey('learning_paths.id'), nullable=False)
