...
class GoalType(db.Model):
    ...
    name = db.Column(db.String(100), unique=True, nullable=False)  # basic_conversation, workplace_english, fluency
    ...
    criteria = db.Column(JSON)  # {activities_count: 20, vocabulary_count: 100, streak_days: 7, assessment_score: 80}
...
class AchievementGoal(db.Model):
    ...
    # Status
    status = db.Column(db.String(50), default='active')  # active, completed, abandoned, paused
    progress_percentage = db.Column(db.Float, default=0.0)
    
    # Goal criteria (can be customized from template)
    criteria = db.Column(JSON)  # Same structure as GoalType.criteria
    current_progress = db.Column(JSON)  # Tracks actual progress {activities_count: 15, vocabulary_count: 80, ...}
    ...
    def to_dict(self):
        return {
            ...
            'days_active': (datetime.utcnow() - self.start_date).days if self.start_date else 0,
            'is_completed': self.status == 'completed',
            'is_overdue': self.target_date and datetime.utcnow() > self.target_date and self.status != 'completed' if self.target_date else False
        }
...
class LevelProgression(db.Model):
    ...
    # Progression trigger
    trigger_type = db.Column(db.String(50))  # goal_completion, assessment, points_threshold
    trigger_id = db.Column(db.Integer)  # ID of the trigger (goal_id, assessment_id, etc.)
