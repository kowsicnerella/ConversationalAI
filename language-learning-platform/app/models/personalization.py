...
class UserGoal(db.Model):
    ...
    learning_focus = db.Column(
        db.String(50), default="conversation"
...
class ProficiencyAssessment(db.Model):
    ...
    )  # initial, periodic, challenge
    questions_asked = db.Column(
        db.JSON
    )  # Store the questions asked (legacy - use questions_data instead)
    questions_data = db.Column(
        db.JSON
    )  # Store the questions for in-progress assessments
    user_responses = db.Column(db.JSON)  # Store user responses
    ai_evaluation = db.Column(
        db.JSON
    )  # Store AI's evaluation (legacy - use evaluation_results instead)
...
class MistakePattern(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    mistake_type = db.Column(
        db.String(50), nullable=False
    )  # grammar, vocabulary, pronunciation, etc.
    mistake_category = db.Column(
        db.String(100)
    )  # verb_tenses, articles, prepositions, etc.
...
class LearningSession(db.Model):
    ...
    session_type = db.Column(
        db.String(50), nullable=False
    )  # chat, guided_conversation, role_play, quiz, flashcard, etc.
...
class DailyChallenge(db.Model):
    ...
    challenge_type = db.Column(
        db.String(50), nullable=False
    )  # conversation_starter, vocabulary, grammar
