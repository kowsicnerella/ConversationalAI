...
class NotificationType(db.Model):
    ...
    )  # e.g., "daily_reminder", "streak_alert"
...
class Notification(db.Model):
    ...
    data = db.Column(
        db.JSON
    )  # Additional notification data (badge info, streak count, etc.)
    priority = db.Column(
...
class UserNotificationSettings(db.Model):
    ...
    daily_reminder_time = db.Column(
        db.Time, default=datetime.strptime("19:00", "%H:%M").time()
    )  # 7 PM
    ...
    def to_dict(self):
        return {
            ...
            "daily_reminder_time": (
                self.daily_reminder_time.strftime("%H:%M")
                if self.daily_reminder_time
                else None
            ),
