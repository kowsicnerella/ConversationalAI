...
class UserFlowTester:
    ...
    def test_initial_assessment(self):
        ...
        try:
            response = requests.get(f"{BASE_URL}/onboarding/assessment", headers=headers)
            if response.status_code == 200:
                data = response.json()
    ...
    def test_learning_path_discovery(self):
        ...
        try:
            response = requests.get(f"{BASE_URL}/learning-paths/recommended", headers=headers)
            if response.status_code == 200:
                data = response.json()
                ...
                for i, path in enumerate(paths[:3], 1):
                    print(f"  {i}. {path.get('title', 'N/A')} - {path.get('difficulty_level', 'N/A')}")
    ...
    def test_activity_generation(self):
        ...
        for activity in activities:
            try:
                response = requests.post(
                    f"{BASE_URL}/activities/generate",
                    json=activity,
                    headers=headers
                )
                ...
                self.log_result(f"Generate {activity['activity_type'].title()} Activity", False, str(e))
...
if __name__ == '__main__':
    ...
    print(f"{Fore.YELLOW}Make sure the Flask server is running on {BASE_URL}{Style.RESET_ALL}\n")
