...
def test_activity_generation():
    ...
    try:
        ...
        print(
            "💡 Note: This might be due to missing GEMINI_API_KEY environment variable"
        )
            ...
            if isinstance(result, dict):
                if "error" in result:
                    ...
                    if test_case["type"] == "quiz" and "questions" in result:
                        ...
                        text_preview = (
                            result["reading_text"][:100] + "..."
                            if len(result["reading_text"]) > 100
                            else result["reading_text"]
                        )
    ...
    for test in test_responses:
        ...
        if isinstance(result, dict):
            ...
            if all(key in actual_keys for key in test["expected_keys"]):
                ...
                print(
                    f"   ⚠️  Unexpected keys - Expected: {test['expected_keys']}, Got: {actual_keys}"
                )
        else:
