...
def create_learning_paths():
    ...
    learning_paths = [
        ...
        },
        {
            ...
            "prerequisites": [
                "Intermediate Telugu conversation",
    ...
    for path_data in learning_paths:
        ...
        path_data["prerequisites"] = (
            str(path_data["prerequisites"]) if path_data["prerequisites"] else "[]"
        )
    ...
    try:
        ...
        categories = (
            db.session.query(LearningPath.category, db.func.count(LearningPath.id))
            .group_by(LearningPath.category)
            .all()
        )
        difficulties = (
            db.session.query(
                LearningPath.difficulty_level, db.func.count(LearningPath.id)
            )
            .group_by(LearningPath.difficulty_level)
            .all()
        )
