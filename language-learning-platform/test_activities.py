...
def test_generate_quiz(token):
    ...
    try:
        response = requests.post(
            ...
            headers=headers,
            json={"topic": "daily routine", "level": "beginner", "num_questions": 5},
        )
        ...
        if response.status_code == 200:
            ...
            if data.get("success") and "data" in data:
                ...
                has_questions = (
                    "questions" in quiz_data and len(quiz_data["questions"]) == 5
                )
        ...
        if response.status_code == 200:
            ...
            if data.get("success") and "evaluation" in data:
                ...
                if has_score and has_points and has_feedback:
                    ...
                    log_test(
                        "Submit Quiz - All Correct", False, "Missing evaluation fields"
                    )
                    return None
            else:
                log_test(
                    "Submit Quiz - All Correct", False, "Invalid response structure"
                )
                return None
        else:
            log_test(
...
def test_generate_flashcards(token):
    ...
    try:
        response = requests.post(
            ...
            headers=headers,
            json={"topic": "food", "level": "beginner", "num_cards": 10},
        )
        ...
        if response.status_code == 200:
            ...
            if data.get("success") and "data" in data:
                ...
                has_cards = (
                    "flashcards" in flashcard_data
                    and len(flashcard_data["flashcards"]) == 10
                )
                has_session_id = "session_id" in flashcard_data

                if has_title and has_cards and has_session_id:
                    log_test(
                        "Generate Flashcards - Basic",
                        True,
                ...
                log_test(
                    "Generate Flashcards - Basic", False, "Invalid response structure"
                )
                return None
        else:
            log_test(
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "user_answers": {"responses": responses},
...
def test_get_topics(token):
    ...
    try:
        response = requests.get(f"{BASE_URL}/activities/topics", headers=headers)
...
def test_get_history(token):
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/activities/history", headers=headers, params={"limit": 10}
        )
        ...
        if response.status_code == 200:
            ...
            if data.get("success") and "history" in data:
                ...
                log_test(
                    ...
                    True,
                    f"Found {len(history)} completed activities",
                )
                return history
            else:
...
def print_summary():
    ...
    success_rate = (
        (test_results["passed"] / len(test_results["tests"]) * 100)
